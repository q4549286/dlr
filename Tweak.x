...
#define EchoLog(format, ...) \
    do { \
        ...
        NSLog(@"[KeChuan-Test-Truth-V6-ScreenLog] %@", logMessage); \
        if (g_screenLogger) { \
...
- (void)viewDidLoad {
    ...
    if (targetClass && [self isKindOfClass:targetClass]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [testButton setTitle:@"测试课传(V6诊断)" forState:UIControlStateNormal];
            ...
            logger.textColor = [UIColor greenColor];
            logger.font = [UIFont monospacedSystemFontOfSize:10 weight:UIFontWeightRegular];
            logger.editable = NO;
            logger.layer.borderColor = [UIColor greenColor].CGColor;
