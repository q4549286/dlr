     ...
     @try { 
         ...
         void (^processLayers)(NSArray *, NSString *) = ^(NSArray *layers, NSString *type) { 
             ...
             for (id layer in layers) { 
                 ...
                 [allLayerInfos addObject:@{ @"type": type, @"text": GetStringFromLayer(layer), @"angle": @(atan2(dy, dx)), @"radius": @(sqrt(dxdx + dydy)) }]; 
         ...
         for (NSDictionary *info in allLayerInfos) { 
             ...
             BOOL foundGroup = NO; for (NSNumber angleKey in [palaceGroups allKeys]) { CGFloat diff = fabsf([info[@"angle"] floatValue] - [angleKey floatValue]); if (diff > M_PI) diff = 2M_PI-diff; if (diff < 0.15) { [palaceGroups[angleKey] addObject:info]; foundGroup=YES; break; } } 
