...
- (void)processKeChuanQueue_Truth {
    if (g_keChuanWorkQueue.count == 0) {
        ...
        // g_capturedKeChuanDetailArray 和 g_keChuanTitleQueue 保留，直到用户复制
        return;
    }
    ...
    // 使用 g_capturedKeChuanDetailArray.count 来定位当前任务的标题
    // 这是因为标题队列是预先填充的，而详情数组是逐步填充的
