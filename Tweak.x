...
static void LogMessage(LogLevel level, NSString *format, ...) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSString *newLogEntry = [NSString stringWithFormat:@"%@%@\n", logPrefix, message];
        UIFont *logFont = [UIFont fontWithName:@"Menlo" size:12] ?: [UIFont systemFontOfSize:12];
        NSAttributedString *newAttributedString = [[NSAttributedString alloc] initWithString:newLogEntry attributes:@{NSForegroundColorAttributeName: color, NSFontAttributeName: logFont}];
