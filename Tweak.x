...
static void SimulateTapAtPoint(CGPoint point) {
    ...
    // 1. 使用 hitTest 找到坐标下的目标视图，这是最标准、最安全的方式
    UIView *targetView = [keyWindow hitTest:point withEvent:nil];
    if (!targetView) {
        NSLog(@"[坐标提取] 错误: 在坐标 (%.1f, %.1f) 处未找到任何可点击的视图。", point.x, point.y);
        return;
    }
    
    // 2. 直接调用目标视图的 touch 事件方法，而不是通过 sendEvent 注入系统
    // 这种方法绕过了复杂的事件队列，更直接也更稳定
...
- (void)viewDidLoad {
    ...
    if (targetClass && [self isKindOfClass:targetClass]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            extractButton.frame = CGRectMake(keyWindow.bounds.size.width - 150, 45 + 80, 140, 50); // 增加高度以容纳两行
            ...
            [extractButton setTitle:@"一键提取\n(稳定版)" forState:UIControlStateNormal];
