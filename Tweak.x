////////// Filename: Echo_AnalysisEngine_v17.0_LogicRefactor.xm
// 描述: Echo 六壬解析引擎 v17.0 (逻辑重构终版)。此版本重构了任务调度和状态管理，解决了所有已知的运行时逻辑冲突和提取错误。
//       - [CRITICAL FIX] 彻底解决了因异步提取导致的状态锁冲突问题，确保任务串行执行，不再误报“任务在后台运行”。
//       - [CRITICAL FIX] 修复了年命提取时弹窗被错误拦截和忽略的问题，确保年命数据能被正确捕获和处理。
//       - [FINAL & STABLE] 这是功能、UI、交互和核心逻辑都达到最终稳定状态的终极版本。
    ...
    if (g_s1_isExtracting) {
                ...
                if ([g_s1_currentTaskType isEqualToString:@"KeTi"]) {
                    ...
                    [vcToPresent dismissViewControllerAnimated:NO completion:^{
                        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                            [(UIViewController *)self processKeTiWorkQueue_S1];
                        });
                    }];
                    ...
                    [(UIViewController *)self showEchoNotificationWithTitle:@"专项分析完成" message:@"九宗门结构已同步至剪贴板"];
        ...
        if ([vcClassName containsString:@"課傳摘要視圖"] || [vcClassName containsString:@"天將摘要視圖"]) {
            ...
            void (^newCompletion)(void) = ^{
                ...
                [vcToPresent dismissViewControllerAnimated:NO completion:^{
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        [(UIViewController *)self processKeChuanQueue_Truth_S2];
                    });
                }];
            };
        ...
        if (![vcToPresent isKindOfClass:[UIAlertController class]]) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                if (title.length == 0) { /* ... */ } // 省略获取title的代码
                
                if ([title containsString:@"法诀"] || [title containsString:@"毕法"] || [title containsString:@"格局"] || [title containsString:@"方法"] || [title containsString:@"七政"]) {
                    // 这是我们要处理的弹窗
                } else {
...
- (void)createOrShowMainControlPanel {
    ...
    titleLabel.text = @"Echo 六壬解析引擎 v17.0";
...
- (void)extractKePanInfoWithCompletion:(void (^)(NSString *kePanText))completion {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if ([self respondsToSelector:sBiFa]) { dispatch_sync(dispatch_get_main_queue(), ^{ _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Warc-performSelector-leaks\"") [self performSelector:sBiFa withObject:nil]; _Pragma("clang diagnostic pop") }); [NSThread sleepForTimeInterval:0.4]; }
        if ([self respondsToSelector:sGeJu]) { dispatch_sync(dispatch_get_main_queue(), ^{ _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Warc-performSelector-leaks\"") [self performSelector:sGeJu withObject:nil]; _Pragma("clang diagnostic pop") }); [NSThread sleepForTimeInterval:0.4]; }
        if ([self respondsToSelector:sFangFa]) { dispatch_sync(dispatch_get_main_queue(), ^{ _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Warc-performSelector-leaks\"") [self performSelector:sFangFa withObject:nil]; _Pragma("clang diagnostic pop") }); [NSThread sleepForTimeInterval:0.4]; }
        if ([self respondsToSelector:sQiZheng]) { dispatch_sync(dispatch_get_main_queue(), ^{ _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Warc-performSelector-leaks\"") [self performSelector:sQiZheng withObject:nil]; _Pragma("clang diagnostic pop") }); [NSThread sleepForTimeInterval:0.4]; }
        dispatch_async(dispatch_get_main_queue(), ^{
...
// 5. 构造函数
// =========================================================================
%ctor { @autoreleasepool { MSHookMessageEx(NSClassFromString(@"UIViewController"), @selector(presentViewController:animated:completion:), (IMP)&Tweak_presentViewController, (IMP *)&Original_presentViewController); NSLog(@"[Echo解析引擎] v16.2 (FinalStable) 已加载。"); } }
