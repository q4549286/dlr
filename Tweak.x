...
static NSString* generateStructuredReport(NSDictionary *reportData) {
   ...
   __block NSInteger sectionCounter = 4;
    ...
    NSString *keTiFull = reportData[@"课体范式_详"] ?: reportData[@"课体范式_简"];
...
- (void)handleMasterButtonTap:(UIButton *)sender {
    ...
    switch (sender.tag) {
        ...
        case kButtonTag_GeJu: {
            [self startS1ExtractionWithTaskType:@"GeJu" includeXiangJie:YES completion:^(NSString *result) {
                __strong typeof(weakSelf) strongSelf = weakSelf; if (!strongSelf) return;
        ...
        case kButtonTag_FangFa: {
            [self startS1ExtractionWithTaskType:@"FangFa" includeXiangJie:YES completion:^(NSString *result) {
                __strong typeof(weakSelf) strongSelf = weakSelf; if (!strongSelf) return;
...
- (void)extractKePanInfoWithCompletion:(void (^)(NSMutableDictionary *reportData))completion {
    ...
    [self extractTimeInfoWithCompletion:^{
        ...
        LogMessage(EchoLogTypeInfo, @"[盘面] 开始无痕解析弹窗类信息 (七政/三宫时)...");
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            SEL sQiZheng = NSSelectorFromString(@"顯示七政信息WithSender:"), sSanGong = NSSelectorFromString(@"顯示三宮時信息WithSender:");
...
- (void)startS1ExtractionWithTaskType:(NSString *)taskType includeXiangJie:(BOOL)include completion:(void (^)(NSString *result))completion {
    ...
    if(g_s1_completion_handler) { [self showProgressHUD:[NSString stringWithFormat:@"正在解析: %@", taskType]]; LogMessage(EchoLogTypeInfo, @"[集成任务] 开始提取 %@ (%@)...", taskType, mode); } 
    ...
    if ([taskType isEqualToString:@"KeTi"]) {
        ...
        NSDictionary *taskSelectorMap = @{
            @"JiuZongMen": @"顯示九宗門概覽",
            @"BiFa": @"顯示法訣總覽",
            @"GeJu": @"顯示格局總覽",
            @"FangFa": @"顯示方法總覽"
        };
        NSString *selectorName = taskSelectorMap[taskType];
        if (selectorName) {
            SEL selector = NSSelectorFromString(selectorName);
            if ([self respondsToSelector:selector]) {
                LogMessage(EchoLogTypeInfo, @"[调用] 正在通过 Selector '%@' 请求数据...", selectorName);
                SUPPRESS_LEAK_WARNING([self performSelector:selector]);
            } else {
                NSString *errorMessage = [NSString stringWithFormat:@"[错误] 当前视图无法响应 '%@'。", selectorName];
                ...
                g_s1_isExtracting = NO; [self hideProgressHUD];
            }
        } else {
...
- (void)extractNianmingInfoWithCompletion:(void (^)(NSString *nianmingText))completion {
    ...
    if (!targetCV) { LogMessage(EchoLogTypeWarning, @"[行年] 未找到行年单元，跳过分析。"); g_isExtractingNianming = NO; if (completion) { completion(@""); } [self hideProgressHUD]; return; }
    ...
    if (allUnitCells.count == 0) { LogMessage(EchoLogTypeWarning, @"[行年] 行年单元数量为0，跳过分析。"); g_isExtractingNianming = NO; if (completion) { completion(@""); } [self hideProgressHUD]; return; }
