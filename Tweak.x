...
//      - [FEATURE] 新增“提取时间”功能，可自动从主界面触发并后台捕获“四时五行”信息。
...
- (void)extractTimeSelectionInfoWithCompletion:(void (^)(NSString *result))completion; // 新增
@end
%hook UILabel
...
- (void)createOrShowMainControlPanel {
    ...
    NSArray *coreButtons = @[
        ...
        @{@"title": @"提取时间", @"icon": @"clock", @"tag": @(kButtonTag_ExtractTimeInfo)} // 新增
    ];
...
- (void)handleMasterButtonTap:(UIButton *)sender {
    ...
    if (g_s1_isExtracting || g_s2_isExtractingKeChuanDetail || g_isExtractingNianming || g_extractedData || g_isExtractingTimeInfo) {
        ...
        case kButtonTag_ExtractTimeInfo: {
            ...
            [self extractTimeSelectionInfoWithCompletion:^(NSString *fullText) {
                ...
                if (startRange.location == NSNotFound) {
                    LogMessage(EchoLogError, @"[解析失败] 未在返回内容中找到“%@”。", startMarker);
                    [strongSelf showEchoNotificationWithTitle:@"解析失败" message:@"数据结构可能已变更。"];
                    return;
                }
                ...
                [strongSelf showEchoNotificationWithTitle:@"提取成功" message:@"时间信息已复制到剪贴板。"];
            }];
            break;
        }
        default: break;
...
- (void)extractTimeSelectionInfoWithCompletion:(void (^)(NSString *result))completion {
    ...
    if (targetViews.count > 0) {
        ...
        LogMessage(EchoLogTypeInfo, @"[自动任务] 找到目标视图 '%@'，正在触发...", targetClassName);

        BOOL didTrigger = NO;
        if (targetView.gestureRecognizers.count > 0) {
            for (UIGestureRecognizer *gesture in targetView.gestureRecognizers) {
                if ([gesture isKindOfClass:[UITapGestureRecognizer class]]) {
                    id targets = [self GetIvarValueSafely:gesture ivarNameSuffix:@"_targets"];
        ...
        if (!didTrigger) {
            LogMessage(EchoLogError, @"[自动任务失败] 无法在'%@'上找到或触发点击事件。", targetClassName);
            if (g_timeInfoCompletionHandler) {
                g_timeInfoCompletionHandler(@"[错误: 无法触发点击]");
            }
            g_isExtractingTimeInfo = NO;
            g_timeInfoCompletionHandler = nil;
        }
        ...
        LogMessage(EchoLogError, @"[自动任务失败] 未能找到视图实例 '%@'。", targetClassName);
        LogMessage(EchoLogTypeWarning, @"请确保您已生成一个课盘。");
        if (g_timeInfoCompletionHandler) {
            g_timeInfoCompletionHandler(@"[错误: 未找到视图实例]");
        }
        g_isExtractingTimeInfo = NO;
        g_timeInfoCompletionHandler = nil;
    }
