...
- (void)viewDidLoad {
    ...
    if (targetClass && [self isKindOfClass:targetClass]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [[window viewWithTag:202501] removeFromSuperview];
            [[window viewWithTag:202502] removeFromSuperview];
            
            UIButton *startButton = [UIButton buttonWithType:UIButtonTypeSystem];
            startButton.frame = CGRectMake(self.view.frame.size.width - 230, 50, 100, 44);
            startButton.tag = 202501;
        ...
        if ([vcClassName containsString:@"課傳摘要視圖"] || [vcClassName containsString:@"天將摘要視圖"]) {
            ...
            void (^scribeCompletion)(void) = ^{
                ...
                void (^tryExtract)(void) = ^{
                    ...
                    if (hasText || retryCount >= 10) { // 如果有文本，或重试10次(1秒)后，执行提取
                        // 【【【采用您提供的精妙排序逻辑】】】
                        ...
                        for (UILabel *label in labels) {
                            if (label.text.length > 0) {
                                [texts addObject:[label.text stringByReplacingOccurrencesOfString:@"\n" withString:@" "]];
                            }
                        }
