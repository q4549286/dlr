...
static NSMutableArray<NSDictionary *> *g_keChuanWorkQueue = nil; // 队列结构改变
...
- (void)viewDidLoad {
    ...
    if (targetClass && [self isKindOfClass:targetClass]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [controlButton setTitle:@"提取(决胜版)" forState:UIControlStateNormal];
            controlButton.titleLabel.font = [UIFont boldSystemFontOfSize:16];
            controlButton.backgroundColor = [UIColor systemRedColor];
        ...
        if ([vcClassName containsString:@"課傳摘要視圖"] || [vcClassName containsString:@"天將摘要視圖"]) {
            ...
            void (^newCompletion)(void) = ^{
                ...
                [viewControllerToPresent dismissViewControllerAnimated:NO completion:^{
                    const double kDelayInSeconds = 0.2; // 0.2秒是一个比较安全且快速的延迟
        ...
        if (containers.count > 0) {
            ...
            for (int i = 0; ivarNames[i] != NULL; ++i) {
                ...
                if (labels.count >= 2) {
                    ...
                    NSDictionary *dizhiTask = @{ @"label": dizhiLabel, @"contextView": keView, @"contextIvarName": @"課視圖" };
                    NSDictionary *tianjiangTask = @{ @"label": tianjiangLabel, @"contextView": keView, @"contextIvarName": @"課視圖" };
...
- (void)processKeChuanQueue_Truth {
    ...
    const char *ivarName = [task[@"contextIvarName"] UTF8String];
    
    LogMessage(@"正在处理: %@", title);
    
    // 1. 手动设置内部状态
    Ivar contextIvar = class_getInstanceVariable([self class], ivarName);
    if (contextIvar) {
        object_setIvar(self, contextIvar, contextView);
        LogMessage(@"成功设置内部状态: ivar '%s' -> %@", ivarName, contextView);
    } else {
        LogMessage(@"警告: 找不到内部变量 '%s'。提取结果可能不正确。", ivarName);
    }
