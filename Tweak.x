...
- (void)performKeChuanDetailExtraction_Final;
...
- (void)viewDidLoad {
    ...
    if (targetClass && [self isKindOfClass:targetClass]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (!keyWindow) { return; }
            NSInteger TestButtonTag = 556700; // Final Tag
            ...
            [testButton addTarget:self action:@selector(performKeChuanDetailExtraction_Final) forControlEvents:UIControlEventTouchUpInside];
...
%new
// --- performKeChuanDetailExtraction_Final: 构建任务队列 ---
- (void)performKeChuanDetailExtraction_Final {
...
%new
// --- processKeChuanQueue_Final: 执行最终的两步操作 ---
- (void)processKeChuanQueue_Final {
    if (g_keChuanWorkQueue.count == 0) {
