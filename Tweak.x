...
return            @"<SYSTEM_PROMPT>\n"
         ...
         @"        *   **【第一条款：动态算子优先裁决】**: 若文本中包含【**得冲神**】这一关键词，系统必须立即将此实体标记为【**S+++级·动态激活**】信号。此标记的法律效力为：【**绝对、无条件地**】覆盖该实体所有与【墓】相关的静态负面解读。后续所有分析，严禁再将此实体作为“墓”论断，而必须、且只能将其解读为【**破墓而出**】、【**困境被外力打破**】的积极动态。\n"
         ...
         @"    *   `鬼` - 同性相克: 负向的压力。`人物`: 小人、敌人。`事件`: 灾祸、疾病、官司、麻烦。（S++++临日德：不论空亡/填实都不做鬼论）\n"
