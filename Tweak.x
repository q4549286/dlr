...
%end
// =========================================================================
// Section 3: 【最终版】一键复制到 AI (已整合并验证年命提取逻辑)
// =========================================================================
        ...
        if ([viewControllerToPresent isKindOfClass:[UIAlertController class]]) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                if ([title containsString:@"法诀"] || [title containsString:@"毕法"] || [title containsString:@"格局"] || [title containsString:@"方法"]) {
                    ...
                    NSMutableArray *allLabels = [NSMutableArray array]; FindSubviewsOfClassRecursive([UILabel class], viewControllerToPresent.view, allLabels); [allLabels sortUsingComparator:^NSComparisonResult(UILabel *o1, UILabel *o2) { return [@(o1.frame.origin.y) compare:@(o2.frame.origin.y)]; }]; for (UILabel *label in allLabels) { if (label.text.length > 0) [textParts addObject:label.text]; } g_extractedData[@"七政四余"] = [textParts componentsJoinedByString:@"\n"];
                } else if ([vcClassName containsString:@"年命摘要視圖"]) {
                    EchoLog(@"抓取到 '年命摘要' 页面: %@", title);
                    NSMutableArray *contentViews = [NSMutableArray array]; FindSubviewsOfClassRecursive([UIView class], viewControllerToPresent.view, contentViews);
                    [contentViews filterUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) { return [evaluatedObject respondsToSelector:@selector(text)]; }]];
                    [contentViews sortUsingComparator:^NSComparisonResult(UIView *v1, UIView *v2) { return [@(v1.frame.origin.y) compare:@(v2.frame.origin.y)]; }];
                    for (UIView *view in contentViews) { NSString *text = [view valueForKey:@"text"]; if (text && text.length > 0) { [textParts addObject:text]; } }
                    NSString *fullContent = [textParts componentsJoinedByString:@"\n"]; NSString *finalNianMingEntry = [NSString stringWithFormat:@"%@\n%@", title, fullContent]; [g_extractedData[@"年命"] addObject:finalNianMingEntry];
                } else { EchoLog(@"抓取到未知弹窗 [%@]，内容被忽略。", title); }
...
- (void)copyAiButtonTapped_FinalMethod {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{
            ...
            if (!unitClass) { EchoLog(@"未找到 '行年單元' 类，跳过年命提取。"); return; }
            ...
            for (UICollectionView *cv in collectionViews) { if (cv.visibleCells.count > 0 && [cv.visibleCells.firstObject isKindOfClass:unitClass]) { targetCollectionView = cv; break; } }
            
            if (!targetCollectionView) { EchoLog(@"未找到包含 '行年單元' 的UICollectionView，跳过年命提取。"); return; }
            id delegate = targetCollectionView.delegate;
            SEL selector = @selector(collectionView:didSelectItemAtIndexPath:);
            if (!delegate || ![delegate respondsToSelector:selector]) { EchoLog(@"CollectionView代理无效，跳过年命提取。"); return; }
            
            NSMutableArray *indexPathsToProcess = [NSMutableArray array];
            for (UICollectionViewCell *cell in [targetCollectionView visibleCells]) {
                if ([cell isKindOfClass:unitClass]) { NSIndexPath *indexPath = [targetCollectionView indexPathForCell:cell]; if (indexPath) [indexPathsToProcess addObject:indexPath]; }
            }
            [indexPathsToProcess sortUsingComparator:^NSComparisonResult(NSIndexPath *obj1, NSIndexPath *obj2) { return [obj1 compare:obj2]; }];
            
            EchoLog(@"找到 %ld 个行年单元，将依次处理。", (long)indexPathsToProcess.count);
            for (NSIndexPath *indexPath in indexPathsToProcess) {
                EchoLog(@"正在处理行年单元 at IndexPath: %@", indexPath);
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            for (NSString *trash in trashList) { biFaOutput = [biFaOutput stringByReplacingOccurrencesOfString:trash withString:@""]; geJuOutput = [geJuOutput stringByReplacingOccurrencesOfString:trash withString:@""]; fangFaOutput = [fangFaOutput stringByReplacingOccurrencesOfString:trash withString:@""]; }
