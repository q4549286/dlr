...
static id GetIvarValueSafely(id object, NSString *ivarName) { if (!object || !ivarName) return nil; Ivar ivar = class_getInstanceVariable([object class], [ivarName UTF8String]); if (ivar) { return object_getIvar(object, ivar); } return nil; }
...
// =========================================================================
// 4. 新的统一功能实现 (New Methods for UIViewController)
// =========================================================================
...
- (void)startSimpleExtraction_Unified {
    ...
    [self _performAdvancedAnalysisExtractionWithCompletion:^(NSString *resultText) {
        NSString *finalResult = [resultText stringByAppendingString:CustomFooterText];
        LogMessage(@"--- [简单提取] 任务完成 ---");
        g_lastResultString = finalResult;
        ...
        [strongSelf _performDetailedKeChuanExtractionWithCompletion:^(NSString *kechuanResult) {
            ...
            if (kechuanResult && kechuanResult.length > 0) {
                ...
                if (qizhengRange.location != NSNotFound) {
                    ...
                    // 如果没找到七政四余，就附加在三传之后
                    NSString *sanchuanMarker = @"末传:";
                    NSRange sanchuanRange = [advancedResult rangeOfString:sanchuanMarker options:NSBackwardsSearch];
                    if (sanchuanRange.location != NSNotFound) {
                         NSRange nextNewlineRange = [advancedResult rangeOfString:@"\n" options:0 range:NSMakeRange(sanchuanRange.location, advancedResult.length - sanchuanRange.location)];
                         NSMutableString *mutableResult = [advancedResult mutableCopy];
                         NSString *insertionText = [NSString stringWithFormat:@"\n\n====================\n【课传详情】\n====================\n\n%@", kechuanResult];
                         [mutableResult insertString:insertionText atIndex:nextNewlineRange.location + nextNewlineRange.length];
                         finalCombinedText = mutableResult;
...
- (void)_startKeChuanExtractionSetup {
    ...
    UIView *keChuanContainer = GetIvarValueSafely(self, @"課傳");
    if (!keChuanContainer) { LogMessage(@"致命错误: '課傳' 总容器视图为nil或找不到ivar。"); g_isExtractingKeChuanDetail = NO; return; }
    ...
    if (sanChuanResults.count > 0) {
        ...
        for (int i = 0; ivarNames[i] != NULL; ++i) {
            UIView *chuanView = GetIvarValueSafely(sanChuanContainer, [NSString stringWithUTF8String:ivarNames[i]]);
    ...
    if (siKeResults.count > 0) {
        ...
        void (^addTaskBlock)(NSString*, NSString*, NSString*) = ^(NSString* ivarName, NSString* fullTitle, NSString* taskType) {
            if (!ivarName) return;
            UILabel *label = GetIvarValueSafely(siKeContainer, ivarName);
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            addTaskBlock(def[@"xiaShen"],   [NSString stringWithFormat:@"%@ - 下神", keTitle], @"diZhi");
            addTaskBlock(def[@"shangShen"], [NSString stringWithFormat:@"%@ - 上神", keTitle], @"diZhi");
            addTaskBlock(def[@"tianJiang"], [NSString stringWithFormat:@"%@ - 天将", keTitle], @"tianJiang");
        }
    }
}
...
- (void)_extractKePanInfoWithCompletion:(void (^)(NSString *kePanText))completion {
    ...
    Class sanChuanViewClass = NSClassFromString(@"六壬大占.三傳視圖");
    if (!sanChuanViewClass) sanChuanViewClass = NSClassFromString(@"六壬大占.傳視圖"); // Fallback for old name
