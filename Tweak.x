...
    }
        ...
        // --- 处理毕法、格局、方法 (目标类名: 格局單元) ---
        if ([vcClassName containsString:@"格局單元"]) {
            LogMessage(EchoLogTypeInfo, @"[无痕捕获] 拦截到 [%@] 弹窗...", title);
            UIView *contentView = vcToPresent.view; // 无痕加载
            ...
            LogMessage(EchoLogTypeSuccess, @"[无痕捕获] 成功解析 [%@]", title);
            isHandled = YES;
        }
        // --- 处理七政四余 (目标类名: 七政信息單元) ---
        else if ([vcClassName containsString:@"七政信息單元"]) {
            LogMessage(EchoLogTypeInfo, @"[无痕捕获] 拦截到 [七政四余] 弹窗...");
            UIView *contentView = vcToPresent.view; // 无痕加载
            ...
            LogMessage(EchoLogTypeSuccess, @"[无痕捕获] 成功解析 [七政四余]");
            isHandled = YES;
        }
        // --- 处理三宫时信息 (目标类名: 三宮信息單元) ---
        else if ([vcClassName containsString:@"三宮信息單元"]) {
            LogMessage(EchoLogTypeInfo, @"[无痕捕获] 拦截到 [三宫时信息] 弹窗...");
            UIView *contentView = vcToPresent.view; // 无痕加载
            ...
            LogMessage(EchoLogTypeSuccess, @"[无痕捕获] 成功解析 [三宫时信息]");
            isHandled = YES;
        }
        ...
        if (isHandled) {
            ...
            // 如果是未知的弹窗，打印日志但让它正常显示，避免破坏未知功能
            LogMessage(EchoLogTypeInfo, @"[捕获] 发现未知弹窗 [%@] (类: %@)，内容已忽略，允许其正常显示。", title, vcClassName);
        }
    }
    // =========================================================================
    // ^^^^^^^^^^^^^^^^ 【核心修改区域结束】 ^^^^^^^^^^^^^^^^
