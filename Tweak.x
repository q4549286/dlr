...
return         @"## v69.0 · 终局版 · 完整Prompt\n"
         ...
         @"\n"
         @"*   **【1.3 节点信息向量化（神将绑定版）】**：我将对【初始网络节点集】中的**每一个地支节点**，进行一次全面的信息绑定，生成其【**初始状态向量**】。此向量是一个**“地支-神将”复合体**，必须包含以下所有维度：\n"
         @"    1.  **地支层信息**:\n"
         @"        *   基础信息：`[节点地支]`\n"
         @"        *   能量评级：`[力与气]` (调用工具箱1.2)\n"
         @"        *   状态标签：`[空亡]`, `[月建]`等 (调用工具箱3.1)\n"
         @"        *   角色标签（多元映射）：相对于T0, T1, T_use...的六亲身份。\n"
         @"        *   位置标签：`[四课位置]`, `[三传位置]`。\n"
         @"    2.  **神将层信息**:\n"
         @"        *   基础信息：`[所乘神将]`\n"
         @"        *   性质解码：该神将的核心职能与吉凶属性（如`白虎`=暴力/裁决, `贵人`=解救/扶持）。\n"
         @"        *   能量约束：该神将是否落在了空亡或休囚死绝之地，导致其能量被削弱。\n"
         ...
         @"    1.  **【微观探寻】**：我从一个【地支-神将复合体】节点出发，探寻其物理关系与神将性质。\n"
         @"    2.  **【即时涌现识别（保留精华）】**：在探寻的同一瞬间，我的识别扫描必须覆盖以下所有层级，以初步识别出宏观模式【**模式A**】：\n"
         @"        *   **A.【落宫格局】**：天盘节点与地盘宫位的组合模式。\n"
         @"        *   **B.【组合格局】**：多节点组合形成的【课体】、【广义格局】或【《毕法赋》条文】。\n"
         @"    3.  **【全局应力测试（核心升级）】**：在采信【模式A】的原始断语之前，我必须立即将其置于一个“虚拟压力场”中，接受来自**全盘所有其他关键信号**的同时审查。我必须进行一次“多维交叉质询”：\n"
         ...
         @"    4.  **【应力测试结论（非标签化叙事）】**：基于以上全局应力测试，我必须输出一段**动态的、叙事性的“压力测试报告”**，来定义【模式A】在当前课盘这个独特生态系统中的**最终真实效应**。\n"
         @"        *   **报告范例**：“初步识别的【亨通课】在应力测试中，因遭遇【三传皆空】的状态性反证，其性质已从‘万事顺利’**转化为**‘一场计划周详、但最终无法落地的纸上富贵’。”\n"
         @"    5.  **【宏观效应反馈】**：只有经过了“全局应力测试”并得出了最终“测试报告”的模式，其被**转化或确认后**的效应，才有资格被反馈并施加于微观作用链之上。\n"
         @"    6.  **【路径效力最终演算（保留精华）】**：我将基于“地支-神将复合体”的完整信息，并结合被反馈的、经过应力测试的**[落宫+组合]格局环境修正**，来计算路径的最终现实效力。\n"
         @"    7.  **【递归与剪枝】**：继续探寻，重复循环。\n"
         @"\n"
         @"##### **第三步：输出统一的、整合性的情报指认**\n"
