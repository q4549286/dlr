...
//
// **`generateStructuredReport` 函数已重构。**
// 核心改动：
// 1.  **引入净化引擎：** 新增 `purifyGeJuContent` 和 `purifyKeChuanDetail` 两个核心净化函数。
// 2.  **重构输出结构：** 报告结构调整为【案卷封面】、【证据清单A/B/C】等司法案卷格式。
// 3.  **剔除结论性文本：** 原有的 `// 4. 爻位详解` 模块被完全废弃，其事实性内容被整合进新的【证据C】。
// 4.  **保留纯净数据：** 所有净化函数都遵循“只保留结构性事实，删除所有解释和断语”的原则。
//
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ END REVIEW ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// ==========================================================================================

static NSString* purifyGeJuContent(NSString *rawContent) {
    if (!rawContent || rawContent.length == 0) return @"";
    
    NSMutableString *purified = [NSMutableString string];
    NSArray *blocks = [rawContent componentsSeparatedByString:@"\n\n"];
    ...
    for (NSString *block in blocks) {
        ...
        NSArray *lines = [trimmedBlock componentsSeparatedByString:@"\n"];
        NSString *firstLine = [[lines.firstObject stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] stringByReplacingOccurrencesOfString:@"\t" withString:@" "];
        
        // 只保留第一行，即格局名称和成因
        if (firstLine.length > 0) {
            [purified appendFormat:@"- %@\n", firstLine];
        }
    }
    
    return [purified stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
}

static NSString* purifyKeChuanDetail(NSString *rawKeChuanDetail) {
    ...
    for (NSTextCheckingResult *match in matches) {
        NSString *objectTitle = [rawKeChuanDetail substringWithRange:[match rangeAtIndex:1]];
        NSString *objectBody = [rawKeChuanDetail substringWithRange:[match rangeAtIndex:2]];
        
        [purifiedReport appendFormat:@"- 对象: %@\n", objectTitle];
        
        NSMutableString *factLog = [NSMutableString string];
        NSArray *lines = [objectBody componentsSeparatedByString:@"\n"];
        BOOL isParsingDunGan = NO;
        ...
        for (NSString *line in lines) {
            ...
            if ([trimmedLine hasPrefix:@"遁干"]) {
                isParsingDunGan = YES;
                [factLog appendString:@"  - 遁干基因:\n"];
                continue;
            }

            if ([trimmedLine hasPrefix:@"杂象"]) {
                isParsingDunGan = NO;
                continue;
            }

            if ([trimmedLine containsString:@"得四时"] || [trimmedLine containsString:@"临"]) {
                 isParsingDunGan = NO;
                 [factLog appendFormat:@"  - 自身状态: %@\n", trimmedLine];
            }
            else if ([trimmedLine hasPrefix:@"合 :"] || [trimmedLine hasPrefix:@"刑 :"] || [trimmedLine hasPrefix:@"冲 :"] || [trimmedLine hasPrefix:@"害 :"] || [trimmedLine hasPrefix:@"破 :"] || [trimmedLine hasPrefix:@"德 :"] || [trimmedLine hasPrefix:@"空 :"]) {
                isParsingDunGan = NO;
                NSString* cleanedInteraction = [[trimmedLine componentsSeparatedByString:@":"] lastObject];
                [factLog appendFormat:@"  - 结构性交互: %@\n", [cleanedInteraction stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]];
            }
            else if (isParsingDunGan && ([trimmedLine hasPrefix:@"一、"] || [trimmedLine hasPrefix:@"二、"])) {
                NSRange separatorRange = [trimmedLine rangeOfString:@"，为日之"];
                if (separatorRange.location != NSNotFound) {
                     [factLog appendFormat:@"    - %@\n", [trimmedLine substringFromIndex:separatorRange.location+1]];
                }
            }
        }
        [purifiedReport appendString:factLog];
        [purifiedReport appendString:@"\n"];
    }
    
    return [purifiedReport stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
}
...
static NSString* generateStructuredReport(NSDictionary *reportData) {
    ...
    if (xunInfo.length > 0) {
        ...
        if (bracketStart.location != NSNotFound && bracketEnd.location != NSNotFound && bracketStart.location < bracketEnd.location) {
            ...
            for (NSString* xunKey in xunKongMap.allKeys) { if ([xunInfo containsString:xunKey]) { xun = [xunKey stringByAppendingString:@"旬"]; kong = xunKongMap[xunKey]; break; } }
    ...
    formatKeyValueSection(@"// 毕法要诀印记", @"毕法要诀");
    formatKeyValueSection(@"// 特定格局印记", @"格局要览");

    // 板块四：证据清单B - 神煞基因库 (净化后)
    NSString *shenShaContent = reportData[@"神煞详情"];
    if (shenShaContent.length > 0) {
        [report appendString:@"// 4. 【证据B：神煞基因库 (待审)】\n"];
        [report appendString:@"// 注意：以下为全盘扫描出的神煞基因，其是否与本案相关、能量是否有效，待审。\n"];
        NSMutableString *formattedShenSha = [NSMutableString string];
        NSArray *lines = [shenShaContent componentsSeparatedByString:@"\n"];
        for (NSString *line in lines) {
            NSString *trimmedLine = [line stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
            if ([trimmedLine hasPrefix:@"//"]) {
                [formattedShenSha appendFormat:@"%@\n", trimmedLine];
            } else if (trimmedLine.length > 0) {
                [formattedShenSha appendFormat:@"- %@\n", [trimmedLine stringByReplacingOccurrencesOfString:@" | " withString:@", "]];
            }
        }
        [report appendString:[formattedShenSha stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
        [report appendString:@"\n\n"];
    }

    // 板块五：证据清单C - 节点交互日志 (净化后)
    NSString *keChuanDetail = reportData[@"课传详解"];
    if (keChuanDetail.length > 0) {
        [report appendString:@"// 5. 【证据C：节点交互日志 (待审)】\n"];
        [report appendString:purifyKeChuanDetail(keChuanDetail)];
        [report appendString:@"\n\n"];
    }
    ...
    if (sanGong.length > 0) {
        [auxiliaryContent appendFormat:@"// 6.2. 三宫时信息\n%@\n\n", purifyGeJuContent(sanGong)]; // 也净化一下
    }

    if (auxiliaryContent.length > 0) {
        [report appendString:@"// 6. 【辅助卷宗：宏观背景 (参考)】\n"];
        [report appendString:[auxiliaryContent stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
        [report appendString:@"\n\n"];
    }
...
static NSString* generateContentSummaryLine(NSString *fullReport) {
    ...
    NSDictionary *keywordMap = @{ 
        ...
        @"// 3. 【证据A：结构性印记 (待审)】": @"结构性印记", 
        @"// 4. 【证据B：神煞基因库 (待审)】": @"神煞基因库", 
        @"// 5. 【证据C：节点交互日志 (待审)】": @"节点交互日志",
        @"// 6. 【辅助卷宗：宏观背景 (参考)】": @"辅助卷宗"
    };
    NSMutableArray *includedSections = [NSMutableArray array];
    NSArray *orderedKeys = @[
        @"// 1. 基础盘元", @"// 2. 核心盘架", @"// 3. 【证据A：结构性印记 (待审)】", 
        @"// 4. 【证据B：神煞基因库 (待审)】", @"// 5. 【证据C：节点交互日志 (待审)】", 
        @"// 6. 【辅助卷宗：宏观背景 (参考)】"
    ];
...
static NSString* formatFinalReport(NSDictionary* reportData) {
    ...
    NSString *footerText = [NSString stringWithFormat:@"\n"
                            "// 强制指令：\n"
                            "// 启动【首席六壬情报分析师】人格，案卷已备齐。\n"
                            "// 案卷编号：20250905-SH-GSIC。你作为主审法官，必须严格遵循你宪法中的【理气归象·终极熔铸协议】，对本案进行三阶审判。\n"
                            "// 禁止任何形式的“多象定一象”式捷径分析。你的最终报告，必须清晰地、可追溯地展示【审理】、【辨气】、【归象】的全过程。\n\n"
                            "// 问题：%@\n", userQuestion];
...
- (void)createOrShowMainControlPanel {
    ...
    NSAttributedString *versionString = [[NSAttributedString alloc] initWithString:@"v28.4" attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:12 weight:UIFontWeightRegular], NSForegroundColorAttributeName: [UIColor lightGrayColor]}];
...
%ctor {
    @autoreleasepool {
        ...
        NSLog(@"[Echo推衍课盘] v28.4 Purified 已加载。");
    }
}
...
static NSString* extractDataFromSplitView_S1(UIView *rootView, BOOL includeXiangJie) {
    ...
    for (UIView *subview in mainStackView.arrangedSubviews) {
        if ([subview isKindOfClass:[UILabel class]]) {
            ...
            if ([text isEqualToString:@"详解"] && !includeXiangJie) { // 关键改动：如果不需要详解，就在这里停止
                break;
            }
