...
return          @"## I/O 标准化协议 V16.0\n"
         ...
         @"                    1.  **启动第一层【通用结构扫描】**:\n"
         @"                        *   **扫描目标**: `官鬼爻`。\n"
         @"                        *   **扫描清单**: `太岁`, `月建`, `月破`, `日辰`, `本命`, `行年`。\n"
         @"                        *   **扫描结果**: 发现`官鬼爻`恰好临`月建`。\n"
         @"                        *   **基因注入**: 为`官鬼爻`打上一个S级标签：`[月建]`。\n"
         @"                        *   **内部报告**: “此‘官鬼’(权力)是当前时令下的核心焦点，能量巨大。”\n"
         @"                    2.  **启动第二层【主题基因扫描】**:\n"
         @"                        *   **判定主题**: 根据提问“测能否晋升”，系统锁定【工作/求官/官司/名誉】主题基因库。其他如[感情]、[求财]等基因库在此次分析中被**完全忽略**。\n"
         @"                        *   **扫描目标**: `官鬼爻`。\n"
         @"                        *   **注入清单**: `将星`, `华盖`, `国印`, `官符`, `天德`, `月德`。\n"
         @"                        *   **扫描结果**: 发现`官鬼爻`同时临`将星`和`国印`。\n"
         @"                        *   **基因注入**: 为`官鬼爻`再打上两个A级标签：`[将星]`、`[国印]`。\n"
         @"                    3.  **生成最终“神煞基因报告”**:\n"
         @"                        *   **最终结论**: 节点`官鬼爻`被注入了以下高权重基因：`[月建 (S级)]`、`[将星 (A级)]`、`[国印 (A级)]`。\n"
         @"                        *   **传递给下一步【假说孵化】**: “这是一个能量处于顶峰（月建）、且天然带有‘领导力’（将星）和‘合法权力’（国印）基因的‘官职’信号。” 后续所有关于此节点的分析，都必须基于这个坚实的基因定性展开。\n"
         @"\n"
         @"**第二步：【假说孵化】**\n"
