...
- (void)triggerGestureOnView:(UIView *)view;
@end
...
- (void)viewDidLoad {
    ...
    if (targetClass && [self isKindOfClass:targetClass]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [controlButton setTitle:@"提取工具(修复版)" forState:UIControlStateNormal];
...
- (void)startExtraction_Truth {
    ...
    if (siKeContainerClass) {
        ...
        if (containers.count > 0) {
            ...
            const char *ivarNames[] = {"第四課", "第三課", "第二課", "第一課", NULL};
            ...
            for (int i = 0; ivarNames[i] != NULL; ++i) {
                ...
                if (ivar) {
                    ...
                    if (keView) {
                        ...
                        if (labels.count >= 2) {
                            UILabel *tianjiangLabel = labels[0];
                            UILabel *dizhiLabel = labels[1];
...
- (void)processKeChuanQueue_Truth {
    ...
    // 【【【核心修正】】】
    [self triggerGestureOnView:itemToClick];
}

%new
// 【【【核心修正：新的点击方法】】】
- (void)triggerGestureOnView:(UIView *)view {
    if (!view || !view.gestureRecognizers.count) {
        LogMessage(@"错误：视图 %@ 没有手势识别器，跳过。", view);
        [g_capturedKeChuanDetailArray addObject:@"[错误: 找不到手势]"];
        [self processKeChuanQueue_Truth];
        return;
    }
    ...
    for (UIGestureRecognizer *gesture in view.gestureRecognizers) {
        ...
        if (targets && targets.count > 0) {
            ...
            // 从中获取真正的target和action
            id target = [gestureTarget valueForKey:@"target"];
            SEL action = (SEL)[gestureTarget valueForKey:@"action"];

            if (target && action && [target respondsToSelector:action]) {
                LogMessage(@"触发手势: %@, Target: %@, Action: %@", gesture, target, NSStringFromSelector(action));
                #pragma clang diagnostic push
                #pragma clang diagnostic ignored "-Warc-performSelector-leaks"
                // 把手势本身作为参数传进去，这才是最真实的模拟
                [target performSelector:action withObject:gesture];
                #pragma clang diagnostic pop
                triggered = YES;
                break; // 假设每个view只有一个有效的点击手势
            }
        }
    }
    
    if (!triggered) {
        LogMessage(@"错误: 未能触发 %@ 上的任何手势动作。", view);
        [g_capturedKeChuanDetailArray addObject:@"[错误: 无法触发手势]"];
        [self processKeChuanQueue_Truth]; // 无论如何都要继续下一个
    }
}
