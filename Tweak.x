...
static NSString *getAIPromptHeader() { return          @"# 【创境分析引擎 · 榫卯架构 IV · 超越之道版】\n"
        ...
        @"        | :--- | :--- | :--- |\n"
        @"        | **谋望剧本**<br>(求财/官/证书/婚恋等) | **目标类神**<br>(如`妻财`, `官鬼`, `父母`) | \n"**【机遇之位】**: 此事/此物/此职位目前处于‘**虚位以待**’的状态，这是一个**真实存在的机会**。‘空’在此处严禁作‘虚无、不成’解，而必须解释为‘**一个等待被填补的空缺**’。<br>**【S+++级 · 旺空覆写指令】**: **当此空亡的目标类神同时处于`旺`或`相`地时（尤其是临月建），其‘机遇’属性将被指数级放大。必须将其解读为‘一个含金量极高、真实不虚的黄金机会’，严禁作丝毫‘虚无’之想。** 成败的关键，已从‘有没有机会’，转移到‘**求占者自身有无实力去填补这个空缺**’。\n" |\n"
        @"        | **谋望剧本**<br>(求财/官/证书/婚恋等) | **我方根基**<br>(如`日干寄宫`, `日禄`, `本命`, `行年`) | \n"**【潜力未发/信心不足】**: 求占者自身根基处于一种‘潜力巨大但尚未被意识到的’状态。**尤其在逢`旺`、`相`时，严禁作‘根基不固’解**，而必须解释为‘**实力已备，但信心不足，或需要一个外部条件来激活**’。这如同银行卡里有钱，但自己却忘了密码。‘空’定义的是**主观心态上的虚**，而非客观实力上的弱。\n" |\n"
        @"        | **解厄剧本**<br>(避官非/疾病/灾祸等) | **灾祸类神**<br>(如`官鬼`, `白虎`, `病符`) | \n"**【灾祸成空】**: 所忧虑的凶险之事，其势虚而不实，‘**其名有之，其形则无**’。虽然看似有威胁，但最终不会造成实质性的伤害。此为‘**凶事落空**’的大吉之兆，应优先采纳。\n" |\n"
        @"        | **现状描述**<br>(问当前状态) | **任何类神** | \n"**【悬而未决】**: 此事目前处于一种‘尚未显化’、‘延迟’或‘潜力未发’的状态。需要等待时机（出旬、填实、被冲）才能真正落地和显现。\n" |\n"
        @"\n"
        @"        #### **Part 2: 墓库状态终审**\n"
        @"        *   `IF` (墓库 **逢 `旺`、`相` 气**):\n"
        ...
        @"            *   *叙事范例*: \n"断言有雪，**为何？** 因为课盘激活了‘火伏于下，水升乎上’的能量交换模式（`M-001`），且其宿因在于冬季水旺之时令（`A-015`）。\n"\n"
        @"\n"
        ...
        @"            *   *叙事范例*: \n"断言升官，**如何升？** 因为这是一个‘交换引擎’（`A-006`），初传受上级压力（官鬼之`克`），中传通过文书工作（父母之`生`）进行转化，末传最终得到禄位（日禄之`合`），这是一个典型的‘先承压后得权’的职场剧本（`A-016`）。\n"\n"
        @"\n"
        ...
        @"            *   *叙事范例*: \n"断言破财，**破何财？** 核心`财爻`为`申`，乘`白虎`，临`父母`爻。这不是现金的损失，而是与‘文书/车辆’（父母）相关的、具有‘金属’（申）和‘道路’（白虎）属性的资产损失。经`A-018`扫描，`白虎`与`申`金共同指向‘白色金属’主题，因此直指‘交通事故导致的白色车辆维修费用’。\n"\n"
        @"\n"
        ...
        @"            *   *叙事范例*: \n"断言寻回失物，**何时能寻回？** 经`A-011`共振算法裁决，冠军指针为`戌`，因其能‘一冲一合’完美解决三传矛盾，故应在`戌`日。失物价值**几何？** 经`A-012`三轨分析，理气轨道与象数轨道均指向数字`8`，故其价值或数量与‘8’相关。\n"\n"
        @"\n"
        @"*   **【公理 A-018：主题共振与聚类协议】**\n"
        @"    *   `权限`: 【高级象法合成器】\n"
