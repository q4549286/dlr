...
#import <substrate.h>
// =========================================================================
// 伪造手势识别器类 (Faker Gesture Recognizer)
// =========================================================================
...
static const NSInteger kButtonTag_TianDiPanDetail   = 306; // <<<< 新增
...
// V2 REPLACEMENT BLOCK - END
        ...
        // 检查是否是我们想要的两种弹窗之一
        if ([vcClassName containsString:@"天將摘要視圖"] || [vcClassName containsString:@"天地盤宫位摘要視圖"]) {
            
            // 使用与课传流注相同的提取逻辑，因为它很通用
            NSMutableArray<NSString *> *finalTextParts = [NSMutableArray array];
            NSMutableArray *allStackViews = [NSMutableArray array];
            FindSubviewsOfClassRecursive([UIStackView class], vcToPresent.view, allStackViews);
            ...
            // 组合结果并继续下一个任务
            [g_tianDiPan_resultsArray addObject:[finalTextParts componentsJoinedByString:@"\n"]];
            LogMessage(EchoLogTypeSuccess, @"[天地盘详解] 成功参详内容 (共 %lu 条)", (unsigned long)g_tianDiPan_resultsArray.count);
            
            // 异步调用处理函数，让UI有机会先关闭弹窗
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)processTianDiPanQueue {
    ...
    CGPoint targetPosition = [task[@"targetPosition"] CGPointValue];
    
    LogMessage(EchoLogTypeInfo, @"[天地盘详解V5] 正在参详: %@ at (%.2f, %.2f)", task[@"title"], targetPosition.x, targetPosition.y);
...
- (void)startExtraction_TianDiPan_Detail_WithCompletion:(void (^)(NSString *result))completion {
    if (g_isExtractingTianDiPanDetail) {
        LogMessage(EchoLogError, @"[错误] 天地盘详解任务已在进行中。");
        return;
    }
    ...
    if (g_tianDiPan_workQueue.count == 0) {
        ...
        g_isExtractingTianDiPanDetail = NO;
        if(completion) completion(@"[任务队列为空]"); [self setInteractionBlocked:NO]; return;
    }
...
currentY += compactButtonHeight + 15;
    ...
    @{@"title": @"天盘详解", @"icon": @"circle.grid.3x3.fill", @"tag": @(kButtonTag_TianDiPanDetail)}
    ];
...
- (void)handleMasterButtonTap:(UIButton *)sender {
    ...
    case kButtonTag_TianDiPanDetail: {
        [self startExtraction_TianDiPan_Detail_WithCompletion:^(NSString *result) {
            __strong typeof(weakSelf) strongSelf = weakSelf; if (!strongSelf) return;
            ...
            NSString *finalReport = formatFinalReport(reportData);
            g_lastGeneratedReport = [finalReport copy];
            [strongSelf showEchoNotificationWithTitle:@"推衍完成" message:@"天地盘详解已生成"];
...
for (NSDictionary *palaceInfo in palaceData) {
    // 为了提取更详细的信息，我们需要重新遍历一下原始分组数据
    // 这有点低效，但为了演示是最清晰的
    NSString *diPanText = palaceInfo[@"diPan"];
    NSDictionary *rawGroupData = nil;

    for (NSNumber *groupAngle in palaceGroups) {
        NSMutableArray *group = palaceGroups[groupAngle];
        for (NSDictionary* layerInfo in group) {
            if ([layerInfo[@"type"] isEqualToString:@"diPan"] && [layerInfo[@"text"] isEqualToString:diPanText]) {
                rawGroupData = @{
                    @"angle": groupAngle,
                    @"layers": group
                };
        ...
        if (rawGroupData) break;
    }

    // 提取每个图层的半径
    CGFloat diPanRadius = 0, tianPanRadius = 0, tianJiangRadius = 0;
    if (rawGroupData) {
        for (NSDictionary* layerInfo in rawGroupData[@"layers"]) {
            if ([layerInfo[@"type"] isEqualToString:@"diPan"]) diPanRadius = [layerInfo[@"radius"] floatValue];
            if ([layerInfo[@"type"] isEqualToString:@"tianPan"]) tianPanRadius = [layerInfo[@"radius"] floatValue];
            if ([layerInfo[@"type"] isEqualToString:@"tianJiang"]) tianJiangRadius = [layerInfo[@"radius"] floatValue];
        }
    }
    
    // 格式化输出，包含原生坐标信息
    [result appendFormat:@"- %@宫: %@(%@)  // RawData -> Angle:%.2f, Radii(D/T/J):[%.1f, %.1f, %.1f]\n",
        palaceInfo[@"diPan"],
        palaceInfo[@"tianPan"],
        palaceInfo[@"tianJiang"],
        [rawGroupData[@"angle"] floatValue], // 分组的角度
        diPanRadius,
        tianPanRadius,
        tianJiangRadius
    ];
...
return [result stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    ...
    return [NSString stringWithFormat:@"天地盘推衍异常: %@", exception.reason];
}
}
