...
- (void)viewDidLoad {
    ...
    if (c && [self isKindOfClass:c]) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(0.5*NSEC_PER_SEC)),dispatch_get_main_queue(),^{
            ...
            [b setTitle:@"推衍天地盘详情" forState:UIControlStateNormal]; b.backgroundColor=ECHO_COLOR_MAIN_BLUE; [b setTitleColor:[UIColor whiteColor]forState:UIControlStateNormal];
...
- (void)processTianDiPanQueue {
    ...
    @try {
        // <<<< 核心修复点 2: 欺骗 sourceView，但使用真实的 plateView 来计算坐标 >>>>
        ...
        // 临时将手势识别器附加到 sourceView 上，这样系统才能正确找到它
        [sourceView addGestureRecognizer:singleTapGesture];
        
        // 注入坐标 (注意：坐标是相对于 plateView 的，所以 locationInView 的第二个参数应该是 plateView)
        ...
        // 恢复手势到原始视图
        [plateView addGestureRecognizer:singleTapGesture];
...
%ctor {
    @autoreleasepool {
        ...
        // 移除多余的 Hook
        NSLog(@"[Echo-V9-Final] 天地盘详情提取工具(最终版)已加载。");
    }
}
