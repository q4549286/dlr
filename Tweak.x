////////// Filename: Echo_AnalysisEngine_v13.4_CompositeFix.xm
// 描述: Echo 六壬解析引擎 v13.4 (深度解构修复版)。
//       - [FIXED] 将“课体范式(简)”与“九宗门结构(简)”的提取流程正确地整合进“深度解构”模式。
//       - [REFACTORED] 重构了 S1 专项分析模块，使其支持完成回调 (completion handler)，以便在其他异步流程中复用。
//       - [IMPROVED] “深度解构”流程更新为 5 个步骤，并提供了更清晰的进度提示。
//       - [STABILITY] 这是最稳定、逻辑最清晰的版本，可作为最终部署版本。
...
static NSString *g_s1_keTiResult_forPowerMode = nil;
static NSString *g_s1_jiuZongMenResult_forPowerMode = nil;
...
- (void)startS1ExtractionWithTaskType:(NSString *)taskType includeXiangJie:(BOOL)include completion:(void (^)(NSString *result))completion; - (void)processKeTiWorkQueue_S1;
...
- (void)createOrShowMainControlPanel {
    ...
    titleLabel.text = @"Echo 六壬解析引擎 v13.4";
...
- (void)handleMasterButtonTap:(UIButton *)sender {
    ...
    switch (sender.tag) {
        ...
        case 202: [self startS1ExtractionWithTaskType:@"KeTi" includeXiangJie:NO completion:nil]; break;
        case 203: [self startS1ExtractionWithTaskType:@"JiuZongMen" includeXiangJie:YES completion:nil]; break;
        case 204: [self startS1ExtractionWithTaskType:@"JiuZongMen" includeXiangJie:NO completion:nil]; break;
...
- (void)startS1ExtractionWithTaskType:(NSString *)taskType includeXiangJie:(BOOL)include completion:(void (^)(NSString *result))completion {
    ...
    g_s1_completion_handler = completion; // 保存回调

    LogMessage(EchoLogTypeTask, @"[任务启动] 模式: %@ (详情: %@)", taskType, include ? @"开启" : @"关闭");

    if ([taskType isEqualToString:@"KeTi"]) {
        UIWindow *keyWindow = GetFrontmostWindow();
        if (!keyWindow) { LogMessage(EchoLogError, @"[错误] 无法找到主窗口。"); g_s1_isExtracting = NO; return; }

        Class keTiCellClass = NSClassFromString(@"六壬大占.課體單元");
        if (!keTiCellClass) { LogMessage(EchoLogError, @"[错误] 无法找到 '課體單元' 类。"); g_s1_isExtracting = NO; return; }
        ...
        if (!g_s1_keTi_targetCV) { LogMessage(EchoLogError, @"[错误] 无法找到包含“课体”的UICollectionView。"); g_s1_isExtracting = NO; return; }
        ...
        for (NSInteger i = 0; i < totalItems; i++) {
            [g_s1_keTi_workQueue addObject:[NSIndexPath indexPathForItem:i inSection:0]];
        }

        if (g_s1_keTi_workQueue.count == 0) {
            LogMessage(EchoLogTypeWarning, @"[警告] 未找到任何“课体”单元来创建任务队列。");
            g_s1_isExtracting = NO;
            if (g_s1_completion_handler) { // 如果队列为空，直接调用回调并返回空字符串
        ...
        if ([self respondsToSelector:selector]) {
            ...
            g_s1_isExtracting = NO;
            if (g_s1_completion_handler) { // 如果无法调用，也返回空
                g_s1_completion_handler(@"");
                g_s1_completion_handler = nil;
            }
        }
...
- (void)processKeTiWorkQueue_S1 {
    if (g_s1_keTi_workQueue.count == 0) {
        ...
        NSString *finalText = [NSString stringWithFormat:@"// 课体范式 (%@)\n\n%@", g_s1_shouldIncludeXiangJie ? @"详" : @"简", [finalResult stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
    ...
    if (delegate && [delegate respondsToSelector:@selector(collectionView:didSelectItemAtIndexPath:)]) {
        ...
        [self processKeTiWorkQueue_S1]; // 继续下一个
    }
}
%new
...
- (void)executeCompositeExtraction {
    ...
    // 清理旧的临时结果
    g_s1_keTiResult_forPowerMode = nil;
    g_s1_jiuZongMenResult_forPowerMode = nil;
    g_s2_baseTextCacheForPowerMode = nil;
    g_s2_finalResultFromKeChuan = nil;

    [self showProgressHUD:@"步骤 1/5: 解析基础盘面..."];
    [self extractKePanInfoWithCompletion:^(NSString *kePanText) {
        ...
        [self updateProgressHUD:@"步骤 2/5: 解析课体范式..."];
        [self startS1ExtractionWithTaskType:@"KeTi" includeXiangJie:NO completion:^(NSString *keTiResult) {
            g_s1_keTiResult_forPowerMode = keTiResult;
            LogMessage(EchoLogTypeSuccess, @"[解构] 课体范式(简)解析完成。");

            [self updateProgressHUD:@"步骤 3/5: 解析九宗门..."];
            [self startS1ExtractionWithTaskType:@"JiuZongMen" includeXiangJie:NO completion:^(NSString *jiuZongMenResult) {
                g_s1_jiuZongMenResult_forPowerMode = jiuZongMenResult;
                LogMessage(EchoLogTypeSuccess, @"[解构] 九宗门(简)解析完成。");

                [self updateProgressHUD:@"步骤 4/5: 推演课传流注..."];
                [self startExtraction_Truth_S2_WithCompletion:^{
                    LogMessage(EchoLogTypeSuccess, @"[解构] 课传流注推演完成。");

                    [self updateProgressHUD:@"步骤 5/5: 分析行年参数..."];
                    [self extractNianmingInfoWithCompletion:^(NSString *nianmingText) {
                        ...
                        // 添加课体和九宗门结果
                        if (g_s1_keTiResult_forPowerMode.length > 0) {
                            [finalResult appendFormat:@"\n\n%@", g_s1_keTiResult_forPowerMode];
                        }
                        if (g_s1_jiuZongMenResult_forPowerMode.length > 0) {
                            [finalResult appendFormat:@"\n\n%@", g_s1_jiuZongMenResult_forPowerMode];
                        }
                        ...
                        if (nianmingText.length > 0) {
                            [finalResult appendFormat:@"\n\n// 行年参数\n\n%@", [nianmingText stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
                        }
...
- (void)extractSinglePopupInfoWithTaskName:(NSString*)taskName {
    ...
    [self extractKePanInfoWithCompletion:^(NSString *kePanText){
        [self hideProgressHUD];
        // Note: 'kePanText' is not directly used here, but calling the function
        // populates g_extractedData which IS used.
...
- (void)extractKePanInfoWithCompletion:(void (^)(NSString *kePanText))completion {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            // For "extractSinglePopupInfo", the caller needs g_extractedData to remain.
            // For other flows, it should be cleared. So we only clear it when a completion block is present.
            if (completion) {
                NSString *resultText = [finalText stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
                completion(resultText);
                 // After the main flows ("标准报告", "深度解构") have used the result, we can clear the state.
                g_extractedData = nil;
...
// 5. 构造函数
// =========================================================================
%ctor { @autoreleasepool { MSHookMessageEx(NSClassFromString(@"UIViewController"), @selector(presentViewController:animated:completion:), (IMP)&Tweak_presentViewController, (IMP *)&Original_presentViewController); NSLog(@"[Echo解析引擎] v13.4 (CompositeFix) 已加载。"); } }
