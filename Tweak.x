...
static NSMutableArray<UIGestureRecognizer *> *g_keChuanWorkQueue = nil; // 队列现在只存手势
...
- (void)viewDidLoad {
    ...
    if (targetClass && [self isKindOfClass:targetClass]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [controlButton setTitle:@"提取(真相版)" forState:UIControlStateNormal];
            controlButton.titleLabel.font = [UIFont boldSystemFontOfSize:16];
            controlButton.backgroundColor = [UIColor colorWithRed:0.0 green:0.4 blue:0.2 alpha:1.0]; // 深绿色
...
- (void)processKeChuanQueue_Truth {
    ...
    if ([self respondsToSelector:actionToPerform]) {
        LogMessage(@"调用方法 %@, 传递手势识别器: %@", NSStringFromSelector(actionToPerform), gestureToTrigger);
        #pragma clang diagnostic push
        #pragma clang diagnostic ignored "-Warc-performSelector-leaks"
        [self performSelector:actionToPerform withObject:gestureToTrigger];
        #pragma clang diagnostic pop
    } else {
        LogMessage(@"错误: 方法 %@ 不存在。", NSStringFromSelector(actionToFro mString(@"顯示課傳摘要WithSender:")));
