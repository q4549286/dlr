...
// 声明一个我们推断存在的排盘数据对象接口
@interface PaiPanObject : NSObject
@property (nonatomic, strong) NSArray<NSString *> *法诀;
@property (nonatomic, strong) NSArray *七政; // 内部可能是对象
@end

// 声明 ViewController 可能拥有一个名为 `排盘` 的属性
@interface UIViewController (PaiPanProperty)
@property (nonatomic, strong) PaiPanObject *排盘;
@end
...
- (void)copyAiButtonTapped_FinalPerfect {
    ...
    // --- 0. 获取核心数据源对象 `排盘` ---
    PaiPanObject *paiPanData = nil;
    if ([self respondsToSelector:@selector(排盘)]) {
        paiPanData = [self valueForKey:@"排盘"];
    }

    // --- 1. 从界面提取基础信息 (这些信息在界面上最可靠) ---
    ...
    // --- 2. 从数据源提取毕法和七政 (实现不点开复制) ---
    NSString *biFa = @"";
    if (paiPanData && [paiPanData respondsToSelector:@selector(法诀)]) {
        NSArray *biFaArray = paiPanData.法诀;
    ...
    NSString *qiZheng = @"";
    if (paiPanData && [paiPanData respondsToSelector:@selector(七政)]) {
        NSArray *qiZhengArray = paiPanData.七政;
        if (qiZhengArray && qiZhengArray.count > 0) {
            // 假设七政数组里是字符串，直接拼接。如果不是，AI会收到对象的描述，也很有用。
        ...
        if(siKeViews.count > 0){
            ...
            if(labels.count >= 12){
                ...
                if (columns.allKeys.count == 4) {
                    ...
                    NSMutableArray *column1 = columns[sortedColumnKeys[0]], *column2 = columns[sortedColumnKeys[1]], *column3 = columns[sortedColumnKeys[2]], *column4 = columns[sortedColumnKeys[3]];
                    [column1 sortUsingComparator:^NSComparisonResult(UILabel *a, UILabel *b) { return [@(a.frame.origin.y) compare:@(b.frame.origin.y)]; }];
                    [column2 sortUsingComparator:^NSComparisonResult(UILabel *a, UILabel *b) { return [@(a.frame.origin.y) compare:@(b.frame.origin.y)]; }];
                    [column3 sortUsingComparator:^NSComparisonResult(UILabel *a, UILabel *b) { return [@(a.frame.origin.y) compare:@(b.frame.origin.y)]; }];
                    [column4 sortUsingComparator:^NSComparisonResult(UILabel *a, UILabel *b) { return [@(a.frame.origin.y) compare:@(b.frame.origin.y)]; }];
                    siKe = [NSMutableString stringWithFormat:@"第一课: %@->%@%@\n第二课: %@->%@%@\n第三课: %@->%@%@\n第四课: %@->%@%@", SafeString(((UILabel*)column4[2]).text), SafeString(((UILabel*)column4[1]).text), SafeString(((UILabel*)column4[0]).text), SafeString(((UILabel*)column3[2]).text), SafeString(((UILabel*)column3[1]).text), SafeString(((UILabel*)column3[0]).text), SafeString(((UILabel*)column2[2]).text), SafeString(((UILabel*)column2[1]).text), SafeString(((UILabel*)column2[0]).text), SafeString(((UILabel*)column1[2]).text), SafeString(((UILabel*)column1[1]).text), SafeString(((UILabel*)column1[0]).text)];
                }
            }
        }
    }
