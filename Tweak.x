    ...
    if ([sender isKindOfClass:[UIGestureRecognizer class]]) {
        ...
        @try {
            ...
            if (touches && [touches isKindOfClass:[NSSet class]] && ((NSSet *)touches).count > 0) {
                ...
                NSLog(@"[蓝图侦察兵] Touch LocationInWindow: %@", NSStringFromCGPoint([touch locationInWindow:touch.window]));
                NSLog(@"[蓝图侦察兵] Touch PreciseLocationInWindow: %@", NSStringFromCGPoint([touch preciseLocationInWindow:touch.window]));
                 // 尝试读取更多私有变量
