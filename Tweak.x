...
// 1. 全局变量与辅助函数
// =========================================================================
#define EchoLog(format, ...) NSLog(@"[KeChuan-Test-Truth-V6-TaskDriven] " format, ##__VA_ARGS__)

static NSInteger const TestButtonTag = 556690;
static BOOL g_isExtractingKeChuanDetail = NO;
// 【V6 核心修改】使用一个包含完整任务信息的队列，而不是分离的队列
static NSMutableArray<NSMutableDictionary *> *g_keChuanTaskQueue = nil; 
static NSInteger g_currentTaskIndex = -1; // 用于追踪当前正在处理的任务
...
// --- 【V6 核心修改】presentViewController 现在直接与当前任务交互 ---
- (void)presentViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    if (g_isExtractingKeChuanDetail && g_currentTaskIndex != -1 && g_currentTaskIndex < g_keChuanTaskQueue.count) {
        ...
        if ([vcClassName containsString:@"課傳摘要視圖"] || [vcClassName containsString:@"天將摘要視圖"]) {
            ...
            void (^newCompletion)(void) = ^{
                ...
                // 【V6 关键】将提取到的内容存入当前任务的字典中
                NSMutableDictionary *currentTask = g_keChuanTaskQueue[g_currentTaskIndex];
                currentTask[@"detail"] = fullDetail;
                
                [viewControllerToPresent dismissViewControllerAnimated:NO completion:^{
                    [self processKeChuanQueue_Truth]; // 处理下一个任务
                }];
            };
...
- (void)performKeChuanDetailExtractionTest_Truth {
    ...
    if (sanChuanContainers.count > 0) {
        ...
        [allChuanViews sortUsingComparator:^NSComparisonResult(UIView *v1, UIView *v2) { return [@(v1.frame.origin.y) compare:@(v2.frame.origin.y)]; }];
        ...
        for (NSUInteger i = 0; i < allChuanViews.count; i++) {
            ...
            if (labels.count >= 2) {
                ...
                // 【V6 关键】创建任务字典并添加到队列
                [g_keChuanTaskQueue addObject:[@{ @"item": dizhiLabel, @"type": @"dizhi", @"title": [NSString stringWithFormat:@"%@ - 地支(%@)", rowTitles[i], dizhiLabel.text] } mutableCopy]];
                [g_keChuanTaskQueue addObject:[@{ @"item": tianjiangLabel, @"type": @"tianjiang", @"title": [NSString stringWithFormat:@"%@ - 天将(%@)", rowTitles[i], tianjiangLabel.text] } mutableCopy]];
            }
        }
    }
    ...
    if (siKeViewClass) {
        ...
        if (skViews.count > 0) {
            ...
            // ... (省略找到labels和分列的代码)
            NSMutableArray *allLabels = [NSMutableArray array]; FindSubviewsOfClassRecursive([UILabel class], siKeContainer, allLabels); NSMutableDictionary *cols = [NSMutableDictionary dictionary]; for (UILabel *label in allLabels) { NSString *key = [NSString stringWithFormat:@"%.0f", roundf(CGRectGetMidX(label.frame))]; if (!cols[key]) { cols[key] = [NSMutableArray array]; } [cols[key] addObject:label]; }
            if (cols.allKeys.count == 4) {
                ...
                for (NSUInteger i = 0; i < sortedKeys.count; i++) {
                    ...
                    if (colLabels.count >= 2) {
                        ...
                        [g_keChuanTaskQueue addObject:[@{ @"item": dizhiLabel, @"type": @"dizhi", @"title": [NSString stringWithFormat:@"%@ - 地支(%@)", colTitles[i], dizhiLabel.text] } mutableCopy]];
                        [g_keChuanTaskQueue addObject:[@{ @"item": tianjiangLabel, @"type": @"tianjiang", @"title": [NSString stringWithFormat:@"%@ - 天将(%@)", colTitles[i], tianjiangLabel.text] } mutableCopy]];
                    }
                }
            }
        }
...
%new
// --- 【V6 核心修改】队列处理器现在只负责驱动和完成 ---
- (void)processKeChuanQueue_Truth {
    g_currentTaskIndex++; // 移动到下一个任务索引

    // 检查是否所有任务都已完成
    if (g_currentTaskIndex >= g_keChuanTaskQueue.count) {
        EchoLog(@"[课传详情] 所有任务处理完毕");
        NSMutableString *resultStr = [NSMutableString string];
        for (NSDictionary *task in g_keChuanTaskQueue) {
            NSString *title = task[@"title"] ?: @"[标题丢失]";
            NSString *detail = task[@"detail"] ?: @"[信息提取失败]";
    ...
    // 获取并执行当前任务
    NSDictionary *task = g_keChuanTaskQueue[g_currentTaskIndex];
