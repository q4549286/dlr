...
#define EchoLog(format, ...) NSLog((@"[EchoAI-Combined-V2] " format), ##__VA_ARGS__)

// --- 全局状态变量 ---
static NSInteger const CombinedButtonTag = 112244; // 新的统一按钮Tag
// 用于 "课盘提取" 过程
...
- (void)viewDidLoad {
    ...
    if (targetClass && [self isKindOfClass:targetClass]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (!keyWindow) { return; }
            if ([keyWindow viewWithTag:CombinedButtonTag]) { return; }
            ...
            combinedButton.backgroundColor = [UIColor colorWithRed:0.8 green:0.2 blue:0.2 alpha:1.0]; // A strong color
...
// --- presentViewController: 合并的无感抓取逻辑 ---
- (void)presentViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    ...
    // 分支2: 正在进行“年命提取”
        ...
        if ([g_currentItemToExtract isEqualToString:@"年命摘要"] && [vcClassName containsString:@"年命摘要視圖"]) {
            UIView *contentView = viewControllerToPresent.view; NSMutableArray *allLabels = [NSMutableArray array]; FindSubviewsOfClassRecursive([UILabel class], contentView, allLabels);
            ...
            void (^newCompletion)(void) = ^{
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    ...
                    if (theTableView && [theTableView.delegate respondsToSelector:@selector(tableView:didSelectRowAtIndexPath:)]) {
                        id<UITableViewDelegate> delegate = theTableView.delegate; Class geJuCellClass = NSClassFromString(@"六壬大占.格局單元");
                        if (geJuCellClass) { for (UITableViewCell *cell in theTableView.visibleCells) { if ([cell isKindOfClass:geJuCellClass]) { NSIndexPath *indexPath = [theTableView indexPathForCell:cell]; if (indexPath) { [delegate tableView:theTableView didSelectRowAtIndexPath:indexPath]; } } } }
                    }
        ...
        [self extractNianmingInfoWithCompletion:^(NSString *nianmingText) {
            ...
            [workingAlert dismissViewControllerAnimated:YES completion:^{
                ...
                if (nianmingText && nianmingText.length > 0) {
                    ...
                    finalCombinedText = kePanText; // 如果没有年命信息，则只复制课盘
                }
...
- (void)extractNianmingInfoWithCompletion:(void (^)(NSString *nianmingText))completion {
    ...
    if (!targetCV) {
        EchoLog(@"年命提取模块：未找到行年单元，跳过此部分。");
        g_isTestingNianMing = NO; if (completion) { completion(@""); } return;
    }
