...
// 2. 主功能区
// =========================================================================
@interface UIViewController (FinalTweak_LayoutFix)
- (void)performKeChuanDetailExtraction_LayoutFix;
- (void)processKeChuanQueue_LayoutFix;
@end
...
- (void)viewDidLoad {
    ...
    if (targetClass && [self isKindOfClass:targetClass]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (!keyWindow) { return; }
            NSInteger TestButtonTag = 556702; // Final Tag
            ...
            [testButton setTitle:@"课传提取(强制刷新)" forState:UIControlStateNormal];
            testButton.titleLabel.font = [UIFont boldSystemFontOfSize:16];
            testButton.backgroundColor = [UIColor systemIndigoColor];
            [testButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            testButton.layer.cornerRadius = 8;
            [testButton addTarget:self action:@selector(performKeChuanDetailExtraction_LayoutFix) forControlEvents:UIControlEventTouchUpInside];
            ...
            void (^newCompletion)(void) = ^{
                ...
                [viewControllerToPresent dismissViewControllerAnimated:NO completion:^{
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        [self processKeChuanQueue_LayoutFix];
                    });
                }];
            };
...
%new
// --- performKeChuanDetailExtraction_LayoutFix: 构建任务队列 ---
- (void)performKeChuanDetailExtraction_LayoutFix {
                ...
                if (ivar) {
                    ...
                    if (chuanView) {
                        ...
                        if (labels.count >= 2) {
                            ...
                            U UILabel *tianjiangLabel = labels[labels.count - 1];
...
%new
// --- processKeChuanQueue_LayoutFix: 执行最终的两步操作 ---
- (void)processKeChuanQueue_LayoutFix {
    if (g_keChuanWorkQueue.count == 0) {
    ...
    if (collectionView) {
        ...
        // 【【【最终的、也是唯一的核心修正：强制刷新UI以触发惰性状态更新】】】
        [self.view layoutIfNeeded];
    }
    
    // 步骤 2: 在UI刷新后，调用正确的显示方法
