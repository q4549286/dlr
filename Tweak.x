////// Filename: Echo_AnalysisEngine_v13.38_FinalCorrectIntercept.xm
// 描述: Echo 六壬解析引擎 v13.38 (最终正确拦截版)。
//      - [FIX] 彻底解决时间提取问题。最终确认为半屏Present弹窗，信息源为UITextView。
//          - `Tweak_presentViewController`中加入了对`時間選擇視圖`类名的精准识别，不再错误地检查标题。
//          - 识别成功后，从其内部的UITextView提取正确的详细时间文本。
//          - 全面恢复了正确的异步链式调用流程，确保了任务执行顺序的正确性。
//      - [STABILITY] 此方案是基于所有FLEX证据的最终正确实现。
...
- (void)createOrShowMainControlPanel {
    ...
    NSAttributedString *versionString = [[NSAttributedString alloc] initWithString:@"v13.38" attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:12], NSForegroundColorAttributeName: [UIColor lightGrayColor]}];
...
%ctor {
    @autoreleasepool {
        ...
        NSLog(@"[Echo解析引擎] v13.36 (TrueFinalPushFix) 已加载。");
    }
}
