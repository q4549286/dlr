...
static NSString* extractDataFromPopup(UIView *contentView) {
    ...
    if (tableViews.count > 0) {
        LogTestMessage(@"检测到 TableView 布局，尝试从数据源提取...");
        UITableView *tableView = tableViews.firstObject;
        NSMutableArray<NSString *> *textParts = [NSMutableArray array];
        
        id dataSource = tableView.dataSource;
        // 猜测数据源可能存储在 VC 的某个属性里
        UIViewController *vc = (UIViewController *)tableView.nextResponder;
        ...
        id dataArray = nil;
        if(vc) {
             // 尝试从 ViewController 的 ivar 中寻找数组
             unsigned int ivarCount;
             Ivar *ivars = class_copyIvarList([vc class], &ivarCount);
             for(unsigned int i = 0; i < ivarCount; i++) {
                 Ivar ivar = ivars[i];
                 const char *typeName = ivar_getTypeEncoding(ivar);
                 if (strcmp(typeName, "@\"NSArray\"") == 0) {
                     id potentialArray = object_getIvar(vc, ivar);
                     if ([potentialArray isKindOfClass:[NSArray class]] && [potentialArray count] > 0) {
                        dataArray = potentialArray;
                        LogTestMessage(@"在 VC 中找到一个名为 %s 的数据数组！", ivar_getName(ivar));
                        break;
                     }
                 }
             }
        ...
        if(dataArray) {
            LogTestMessage(@"成功从 VC 数据源数组提取，共 %lu 项", (unsigned long)[dataArray count]);
            for(id item in dataArray) {
                 // 假设每个 item 是一个字典或自定义对象
                 // 我们需要猜测 key 的名字，常见的有 "title", "name", "desc", "content" 等
                 NSString *title = [item valueForKey:@"標題"] ?: [item valueForKey:@"title"];
                 NSString *content = [item valueForKey:@"內容"] ?: [item valueForKey:@"content"];
                 if(title && content) {
        ...
        LogTestMessage(@"从 VC 数据源提取失败，回退到提取所有 UILabel (包括隐藏的)...");
        // 如果无法从数据源提取，使用备用方案：提取所有UILabel，包括隐藏的
        [textParts removeAllObjects];
...
// ... 接口声明
- (void)runEchoNoPopupExtractionTests;- (void)extractJiuZongMen_NoPopup_WithCompletion:(void (^)(NSString *))completion;- (void)extractBiFa_NoPopup_WithCompletion:(void (^)(NSString *))completion;- (void)extractGeJu_NoPopup_WithCompletion:(void (^)(NSString *))completion;- (void)extractFangFa_NoPopup_WithCompletion:(void (^)(NSString *))completion;- (void)extractQiZheng_NoPopup_WithCompletion:(void (^)(NSString *))completion;- (void)extractSanGong_NoPopup_WithCompletion:(void (^)(NSString *))completion;
@end
static void (*Original_presentViewController)(id, SEL, UIViewController *, BOOL, void (^)(void));
static void Tweak_presentViewController(id self, SEL _cmd, UIViewController *vcToPresent, BOOL animated, void (^completion)(void)) { /* ... 与上一版完全相同 ... */ NSString *vcClassName = NSStringFromClass([vcToPresent class]); void (^handleExtraction)(NSString *, NSString *, void(^)(NSString*)) = ^(NSString *taskName, NSString *result, void(^completionBlock)(NSString*)) { [UIPasteboard generalPasteboard].string = result ?: @""; LogTestMessage(@"提取成功！%@ 内容已复制 (共 %lu 字符)", taskName, (unsigned long)(result.length)); if (completionBlock) { completionBlock(result); } }; void (^delayedExtraction)(void(^)()) = ^(void(^extractionLogic)()) { dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), extractionLogic); }; if (g_isExtractingJiuZongMen && [vcClassName containsString:@"課體概覽視圖"]) { LogTestMessage(@"匹配成功 -> 九宗门"); g_isExtractingJiuZongMen = NO; delayedExtraction(^{ NSString *result = extractDataFromPopup(vcToPresent.view); handleExtraction(@"九宗门", result, g_jiuZongMen_completion); g_jiuZongMen_completion = nil; }); return; } else if ([vcClassName containsString:@"格局總覽視圖"]) { if (g_isExtractingBiFa) { LogTestMessage(@"匹配成功 -> 毕法要诀"); g_isExtractingBiFa = NO; delayedExtraction(^{ NSString *result = extractDataFromPopup(vcToPresent.view); handleExtraction(@"毕法要诀", result, g_biFa_completion); g_biFa_completion = nil; }); return; } else if (g_isExtractingGeJu) { LogTestMessage(@"匹配成功 -> 格局要览"); g_isExtractingGeJu = NO; delayedExtraction(^{ NSString *result = extractDataFromPopup(vcToPresent.view); handleExtraction(@"格局要览", result, g_geJu_completion); g_geJu_completion = nil; }); return; } else if (g_isExtractingFangFa) { LogTestMessage(@"匹配成功 -> 解析方法"); g_isExtractingFangFa = NO; delayedExtraction(^{ NSString *result = extractDataFromPopup(vcToPresent.view); handleExtraction(@"解析方法", result, g_fangFa_completion); g_fangFa_completion = nil; }); return; } } else if (g_isExtractingQiZheng && [vcClassName containsString:@"七政"]) { LogTestMessage(@"匹配成功 -> 七政四余"); g_isExtractingQiZheng = NO; delayedExtraction(^{ NSString *result = extractDataFromSimpleLabelPopup(vcToPresent.view); handleExtraction(@"七政四余", result, g_qiZheng_completion); g_qiZheng_completion = nil; }); return; } else if (g_isExtractingSanGong && [vcClassName containsString:@"三宮時信息視圖"]) { LogTestMessage(@"匹配成功 -> 三宫时信息"); g_isExtractingSanGong = NO; delayedExtraction(^{ NSString *result = extractDataFromSimpleLabelPopup(vcToPresent.view); handleExtraction(@"三宫时信息", result, g_sanGong_completion); g_sanGong_completion = nil; }); return; } LogTestMessage(@"弹窗 %@ 未被拦截，正常显示。", vcClassName); Original_presentViewController(self, _cmd, vcToPresent, animated, completion); }
...
// ... (所有 %new 方法和 viewDidLoad hook 与上一版完全相同) ...
