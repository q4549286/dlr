...
// ... (FindSubviewsOfClassRecursive, GetIvarValueSafely, GetStringFromLayer 函数保持不变) ...
...
- (void)presentViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    if (g_extractedData && ![viewControllerToPresent isKindOfClass:[UIAlertController class]]) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (title.length == 0 && allLabels.count > 0) {
                 title = ((UILabel*)allLabels.firstObject).text;
                 titleLabel = allLabels.firstObject;
            }
            ...
            if ([vcClassName containsString:@"七政"]) {
                ...
                // 这是核心修正逻辑
                NSMutableArray *contentLabels = [allLabels mutableCopy];

                // 1. 查找并移除标题Label，避免干扰内容排序
                if (titleLabel) {
                    [contentLabels removeObject:titleLabel];
                } else {
                    // 如果标题是从vc.title获取的，也需要从labels中移除对应的label
                    NSInteger titleLabelIndex = [contentLabels indexOfObjectPassingTest:^BOOL(UILabel *obj, NSUInteger idx, BOOL *stop) {
                        return [obj.text isEqualToString:title];
                    }];
                    if (titleLabelIndex != NSNotFound) {
                        [contentLabels removeObjectAtIndex:titleLabelIndex];
                    }
                }
                
                // 2. 对所有内容标签进行精确排序：首先按行（Y坐标），然后按列（X坐标）
                // 使用一个小的容差（例如5.0f）来处理Y坐标可能存在的微小差异
                [contentLabels sortUsingComparator:^NSComparisonResult(UILabel *o1, UILabel *o2) {
                    if (fabs(o1.frame.origin.y - o2.frame.origin.y) > 5.0f) {
                        return [@(o1.frame.origin.y) compare:@(o2.frame.origin.y)]; // 按Y坐标排序
                    }
                    return [@(o1.frame.origin.x) compare:@(o2.frame.origin.x)]; // Y坐标相近时，按X坐标排序
                }];

                // 3. 健壮的配对逻辑：按顺序两个一组进行配对
                for (NSUInteger i = 0; i < contentLabels.count; i += 2) {
                    if (i + 1 < contentLabels.count) {
                        UILabel *leftLabel = contentLabels[i];
                        UILabel *rightLabel = contentLabels[i+1];
                        [textParts addObject:[NSString stringWithFormat:@"%@: %@", leftLabel.text, rightLabel.text]];
                    } else {
                        // 如果剩下最后一个不成对的标签，也将其加入，避免数据丢失
                        UILabel *singleLabel = contentLabels[i];
                        [textParts addObject:singleLabel.text];
                         EchoLog(@"发现一个不成对的标签: %@", single-Label.text);
                    }
                }
                ...
                if ([title containsString:@"格局"]) {
                    g_extractedData[@"格局"] = content; EchoLog(@"成功抓取并重排版 [格局] 内容 (已修复)");
                } else if ([title containsString:@"方法"]) {
                    g_extractedData[@"方法"] = content; EchoLog(@"成功抓取并重排版 [方法] 内容 (已修复)");
                } else {
                    g_extractedData[@"毕法"] = content; EchoLog(@"成功抓取并重排版 [毕法] 内容 (已修复)");
                }
            } else {
...
// %new 方法 (extractTextFromFirstViewOfClassName, extractTianDiPanInfo_V18, copyAiButtonTapped_FinalMethod) 保持不变
%new
- (NSString *)extractTextFromFirstViewOfClassName:(NSString *)className separator:(NSString *)separator {
