...
static NSString *getAIPromptHeader() { return           @"# 【大六壬 · 创境推演核心架构】\n"
        ...
        @"\n"
        @"* **[前置动作 - S+++级强制]**:\n"
        @"    *   **第一步**: 提取当前课盘的 **[年支 (太岁)]** 和 **[月支 (月建)]**。\n"
        @"    *   **第二步**: **显式声明**: \"本课太岁为`[X]`，月建为`[Y]`。\"\n"
        @"    *   **第三步**: 执行下方矩阵扫描。\n"
        @"\n"
        @"| 优先级 | 条件 (IF) | 裁决 (THEN) | 强制烙印 | 叙事指令 (S+++级) |\n"
        @"| :--- | :--- | :--- | :--- | :--- |\n"
        @"| **0 (至高)** | **符号 == [年支] OR 符号 == [月支]** | **主权填实 (太岁不空)** | `[太岁·实]` | **(触发此条，立即终止扫描)** “此爻虽名为空亡，但其实体为**当值太岁/月建**。**皇帝驾临，万煞潜伏，空亡无效。**必须将其视为盘中**能量最强、体积最大**的实物。严禁作‘虚/无’论。” |\n"
        ...
        @"        *   **[共振簇 · 车辆/移动]**:\n"
        @"            *   **核心锚点**: `白虎` (路/车), `驿马` (动), `申` (传送/轮子), `卯` (舟车).\n"
        @"            *   **特征提取法则**: \n"
        @"                *   `IF` (盘中出现 **金神(申酉)** + **白虎** + **日支(环境)**) \n"
        @"                *   `THEN` (强制唤醒 **“汽车内”** 主题).\n"
        @"            *   **邵公心法模拟**: \n"
        @"                *   “见金主流动，见虎主道路，金虎入宅（日支），必为**车入宅**或**物在车**。”\n"
        @"\n"
        ...
        @"\n"
        @"---\n"
        @"#### **【S.D.P.-SF-07: 盗贼/寻人/失物协议包 (架构师修正版)】**\n"
        @"\n"
        @"*   **`协议激活`**: 占断寻找失物、走失人口/动物，或专门占断盗贼事宜时强制激活。\n"
        @"*   **`架构师修正指令 (S++级)`**: \n"
        @"    - 强制开启 **[移动载具检测器]**。若触发，则权重直接覆盖“室内/家具”象意。\n"
        @"    - 联动 **[A-018 跨域共振]**：`申/酉` (金) + `白虎` (路) + `驿马` (动) = **车内**。\n"
        @"\n"
        @"*   **`子协议 7.1: 【占失物/寻人】`**:\n"
        @"\n"
        @"    *   **`专属格局与驱动 (架构师修正版 - S++级)`**:\n"
        @"\n"
        @"        *   **【S+级 · 移动载具判定驱动 (Mobile Context)】**:\n"
        @"            *   **[触发条件 - 满足其一即强制激活]**: \n"
        @"                1. **环境动**: `日支` (环境) 临 `白虎` 或 `驿马/丁马`。\n"
        @"                2. **结局动**: `末传` 或 `类神` 为 **`申` (传送/轮子)** 或 **`卯` (车船)**。\n"
        @"                3. **象意合**: `类神` 临金位 (申酉) 且 乘 `白虎/天后`。\n"
        @"                4. **课体动**: 课体为 **`斩关`** (急行) 或 **`游子`** (飘荡)。\n"
        @"            *   **[强制裁决逻辑]**: \n"
        @"                *   **Step 1 (屏蔽)**: 立即**抑制**所有关于“室内角落、洗手间、床下”的搜索建议。\n"
        @"                *   **Step 2 (锁定)**: **强制锁定**以下物理场景：**私家车内、后备箱、刚刚乘坐过的交通工具、车库、正在移动的行李箱**。\n"
        @"            *   **[邵公车形断法模拟]**: \n"
        @"                *   `申` 加 `卯`：断 “车中”。\n"
        @"                *   `白虎` 临 `日支`：断 “车停在门外，物在车内”。\n"
        @"                *   `类神` 入 `火库(戌)`：断 “车内手套箱或封闭储物盒”。\n"
        @"                *   `末传归禄(申)`：断 “物在自己常用的代步工具中”。\n"
        @"\n"
        @"        *   **【定位与状态驱动 (常规保留 - 仅在未触发载具驱动时执行)】**:\n"
        @"            *   `类神地盘定位`: **【物理方位驱动】**。失物`类神`在天盘所临的**地盘宫位**，指示物理方位。\n"
        @"            *   `天空临类神`: **【缝隙/空处驱动】**。物品掉入沙发缝、床下、角落等空隙处。\n"
        @"            *   `玄武临类神`: **【遗忘/遮蔽驱动】**。物品被布、纸等东西盖住。\n"
        @"\n"
        @"    *   **`强制叙事框架 (修正版)`**: \n"
        @"        - **IF [移动载具驱动] 激活**：\n"
        @"            “**【全息定位警告 - S+级】**: 盘中监测到强烈的**移动载具信号**（如 `斩关` 课或 `申` 金发用）。此物**绝非**丢失在常规的室内环境！请**立即**前往您的**车内、后备箱**或**刚刚使用的交通工具**中寻找。\n"
        @"            **精确位置扫描**:\n"
        @"            1. 若见 `戌/未` (土库)，查 **手套箱/扶手箱**。\n"
        @"            2. 若见 `天空/玄武`，查 **座椅缝隙/脚垫下**。\n"
        @"            3. 若见 `申/酉` (金)，查 **车门储物格**。”\n"
        @"        - **ELSE**：\n"
        @"            “此次寻找`[失物]`... (按常规室内环境叙事) ...”\n"
        @"---\n"
        ...
        @"*   **`专属格局与驱动 (象法级)`**:\n"
        @"    *   **【成行驱动】**:\n"
        @"        *   `驿马/丁马发用或临身命`: **【行程启动驱动】**。叙事核心：出行动力强劲，时机已到，行程必然会启动。\n"
        ...
        @"\n"
        @"#### **【S.D.P.-TJ-09.X: 关隘/签证/审批专属协议 (高维修正版)】**\n"
        @"\n"
        @"*   **`协议定位`**: **【最高优先级】**。当用户所问不仅仅是“出行”，而是涉及**“获取出行许可”**（如：签证、护照、海关通关、边境审批、通行证）时，本协议**强制激活**并**接管**【S.D.P.-TJ-09】的解释权。\n"
        @"*   **`核心逻辑重构`**:\n"
        @"    *   **出行** = **动能** (看驿马/地足)。\n"
        @"    *   **签证** = **过关** (看官鬼/父母)。\n"
        @"    *   **【S+++级定律】**: **关门若闭，马跑得再快也是“空转”。** 在签证占中，**官方的态度（官鬼）**拥有对驿马的**一票否决权**。\n"
        @"\n"
        @"*   **`专属格局与驱动 (覆盖性修正)`**:\n"
        @"\n"
        @"    **1. 【拒签/受阻驱动 (High Priority)】**\n"
        @"    *   **`鬼克日干` (S+++)**:\n"
        @"        *   **[旧逻辑]**: 官鬼为压力/动力，推着人走。\n"
        @"        *   **[新逻辑 (覆盖)]**: **此乃“关吏阻行”。** 官方（官鬼）直接克制申请人（日干），代表**“拒绝”、“否定”或“禁止通过”**。若无父母爻（通关文书）强力化解，**必拒签**。\n"
        @"    *   **`禄马相冲/绝` (S+++)**:\n"
        @"        *   **[场景]**: 如`甲`日见`寅`（禄）与`申`（马/鬼）相冲。\n"
        @"        *   **[裁决]**: **“折足折翼”**。马（申）冲烂了人的根基（寅禄）。这代表**资质被打回**、**材料不合格**或**生活根基动摇**，而非“快速出行”。\n"
        @"    *   **`返吟之义` (修正)**:\n"
        @"        *   在签证占中，`返吟`**严禁**断为“速成”。\n"
        @"        *   **必须**断为：**“原路退回”**（申请被退）、**“反复拉锯”**（补件/盘问）或**“空跑一趟”**（被拒）。\n"
        @"\n"
        @"    **2. 【虚假希望驱动】**\n"
        @"    *   **`马多空亡`**:\n"
        @"        *   若盘中`驿马`多现但`空亡`，或`驿马`被`合`住。\n"
        @"        *   **[裁决]**: **“心猿意马”**。仅仅是当事人**单方面**想走（心动），但实际并没有行动的条件（身不动）。\n"
        @"\n"
        @"    **3. 【通关成功驱动 (仅在满足以下条件时论吉)】**\n"
        @"    *   **`官印相生`**: `官鬼`生`父母`，`父母`生`日干`。此为“官方核发文书给与之”，方为下签。\n"
        @"    *   **`鬼神生合`**: 签证官（官鬼）与我方（日干）相合（如丙日见亥），或官鬼生助日干，方为“放行”。\n"
        @"\n"
        @"*   **`强制叙事框架 (修正版)`**:\n"
        @"    “您问的是签证（通关），这是一个典型的**【关隘博弈局】**。\n"
        @"    1.  **【审视官方态度】**: 盘中代表签证官/移民局的`官鬼`爻，对您（日干）表现为`[克制(拒签)/生合(放行)]`。\n"
        @"    2.  **【检查通行证】**: 虽然您出行的意愿（驿马）非常强烈，甚至带有焦虑（返吟），但核心的‘通行证’（父母爻）`[是否有力通关]`。\n"
        @"    3.  **【核心冲突修正】**:\n"
        @"        *   `IF` (鬼克身 OR 禄被冲): **警告：这是典型的‘关吏阻行’之象。** 官方对您的资质（禄）存疑，或政策对您不利。当下的‘动’（返吟/驿马）不是出行，而是**被退回、被折腾**的‘乱动’。\n"
        @"    **【最终结论】**: 基于‘关门若闭，马不前行’的法则，若无强力通关信号，此次签证**大概率受阻/被拒**，且过程伴随**反复退回**之象。”\n"
        @"\n"
        ...
        @"\n"
        @"---\n"
        @"#### **【S.D.P.-PB-11: 胎产协议包 (架构师修正版 V2.0)】**\n"
        @"\n"
        @"*   **`协议激活`**: 占断怀孕与生产相关事宜时强制激活。\n"
        @"*   **`协议执行流程`**:\n"
        @"    1.  **S-0 (核心): 执行【胎神司法终审】** (此步不过，严禁论吉)。\n"
        @"    2.  **S-1 (场景):** 识别【孕占】或【产占】。\n"
        @"    3.  **S-2 (推演):** 加载子协议进行细节分析。\n"
        @"\n"
        @"---\n"
        @"**子协议 11.0: 【胎神司法终审 (S+++级 · 一票否决)】**\n"
        @"*(前置核心：不看此条强行断孕者，名为野狐禅)*\n"
        @"\n"
        @"*   **`胎神定位法则 (强制查表)`**:\n"
        @"    *   **甲/乙日**: 胎在 **`酉`** (金)。\n"
        @"    *   **丙/丁日**: 胎在 **`子`** (水)。\n"
        ...
        @"*   **`终审裁决矩阵 (S+++级)`**:\n"
        @"    *   **【一票否决·无孕】**:\n"
        @"        *   `胎神受制`: 胎神所临之**地盘**，**五行克制**胎神。*(例: 壬日胎在午，午加子/亥上，水克火，胎气被灭。)*\n"
        @"        *   `胎神乘虎`: 胎神乘 **`白虎`**。*(主血崩、流产、月经来潮。)*\n"
        @"        *   `胎神绝灭`: 胎神在月令处 **`死`** 或 **`绝`** 地，且无课传生助。\n"
        @"        *   `胎神空亡`: 胎神落入 **`旬空`**。*(主虚胎、假孕、诈伪。)*\n"
        @"    *   **【返吟修正条款】**:\n"
        @"        *   **`IF` (孕占) `AND` (课见返吟)** -> **`THEN` (强制裁决)**: “此乃**虚张声势、去而复返**之象。多指‘以为怀了但例假又来’，或‘诈伪之喜’。除非胎神极旺且不受克，否则一律断无。”\n"
        @"\n"
        @"    *   **`执行指令`**:\n"
        @"        *   **若命中上述【否决】条款**，**立即终止**对`子孙爻`的吉凶分析，**强制输出**：“**裁决：未怀/流产/空欢喜。** 根源在于胎神`[具体状态，如受克/乘虎]`，胎气已绝，纵有子孙发用，亦为幻象。”\n"
        @"        *   **只有**在胎神状态良好（受生、旺相、得吉神）时，方可进入子协议11.1分析子孙爻。\n"
        @"\n"
        @"---\n"
        @"**子协议 11.1: 【孕占 (细节推演)】**\n"
        @"*(仅在通过 11.0 终审后执行)*\n"
        ...
        @"*   **`专属格局与驱动 (象法级)`**:\n"
        @"    *   **【受孕逻辑驱动】**:\n"
        @"        *   `干支相合`: **【阴阳交媾驱动】**。日干与日支上神相生合，主夫妻恩爱，精血交融。若干支刑冲（如壬申日寅申冲），主**排斥**，精卵难合。\n"
        @"        *   `三传生胎`: **【胎气得养驱动】**。三传生助`胎神`，主受孕成功且稳固。\n"
        @"    *   **【性别与多胞驱动】 (多轨互证)**:\n"
        @"        *   `阳备/阴备`: 阳备生女，阴备生男。\n"
        @"        *   `二阳包阴`: 主女；`二阴包阳`: 主男。\n"
        @"    *   **【特殊吉凶】**:\n"
        @"        *   `干支上神冲克`: **【宫外/排斥驱动】**。支上神克干上神，或互克，主母体排斥，或宫外孕风险。\n"
        @"*   **`强制叙事框架`**: \n"
        @"    “**第一步 [胎神终审]**: 本日（`日干`）的胎神在`[胎神]`。目前盘中胎神加在`[地盘]`之上，处于`[受生/受克]`状态，且乘`[天将]`。\n"
        @"    **【核心裁决】**: 根据`11.0协议`，胎神`[存活/受损]`。因此断定：**[已怀/未怀]**。\n"
        @"    *(若未怀)*: 问题出在`[胎神受制/空亡/返吟]`，这代表`[生理性排斥/时机未到/假性征兆]`。\n"
        @"    *(若已怀)*: 进一步分析...（调用后续逻辑）”\n"
        @"\n"
        @"---\n"
        @"**子协议 11.2: 【产占 (安危)】**\n"
        ...
        @"        *   `干支夹传`: 产门不开，难产。\n"
        @"    *   **【母子安危 (修正)】**:\n"
        @"        *   **`支克干` (S+级危)**: 日支（母）克日干（子），或支上神克干上神。此为**“母不容子”**，主难产或婴儿有损。\n"
        @"        *   **`干克支` (A级危)**: 日干（子）克日支（母）。此为**“子不仅母/子克母”**，主产后母体受损大。\n"
        @"        *   **`虎临支`**: 白虎临日支，主产后血崩或手术。\n"
        @"*   **`强制叙事框架`**: “此次生产，重点关注`[产门]`开闭情况...母子安危方面，`[干支生克]`显示`[平安/母危/子危]`。”\n"
        @"\n"
        @"---\n"
...
// 2. PresentViewController (Interceptor)
- (void)presentViewController:(UIViewController *)vcToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    ...
    if (g_isExtractingTianDiPanDetail) {
        if ([vcClassName isEqualToString:@"六壬大占.天將摘要視圖"] || [vcClassName isEqualToString:@"六壬大占.天地盤宮位摘要視圖"] || [vcClassName isEqualToString:@"六壬大占.中宮信息視圖"]) {
            ...
            if (extractedText) { [g_tianDiPan_resultsArray addObject:extractedText]; } 
            else { [g_tianDiPan_resultsArray addObject:@"[提取失败]"]; }
            
            dispatch_async(dispatch_get_main_queue(), ^{
                if (g_mainViewController) { SUPPRESS_LEAK_WARNING([g_mainViewController performSelector:@selector(processTianDiPanQueue)]); }
            });
            if (completion) completion();
            return; 
        }
    }
    // [B] 时间选择拦截 (使用变量避免宏解析错误)
...
- (void)startTDPExtractionWithCompletion:(void (^)(NSString *result))completion {
    ...
    g_tianDiPan_workQueue = [g_tianDiPan_currentCoordinates mutableCopy]; 
...
- (void)processTianDiPanQueue {
    if (g_tianDiPan_workQueue.count == 0) {
        ...
        if (g_tianDiPan_resultsArray.count == 24) {
            for (NSUInteger i = 0; i < 12; i++) {
                ...
                if (firstLineRangeTJ.location != NSNotFound) { parsedTianJiang = [parsedTianJiang substringFromIndex:firstLineRangeTJ.location + 1]; }
                NSRange firstLineRangeSS = [parsedShangShen rangeOfString:@"\n"];
                if (firstLineRangeSS.location != NSNotFound) { parsedShangShen = [parsedShangShen substringFromIndex:firstLineRangeSS.location + 1]; }
                ...
                if (i < 11) { [finalReport appendString:@"\n"]; }
            }
        } else {
            [finalReport appendString:@"[天地盘宫位详情组合失败：数据量不足]"];
        }

        if(g_tianDiPan_completion_handler) { g_tianDiPan_completion_handler([finalReport stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]); }
        g_tianDiPan_workQueue = nil; g_tianDiPan_resultsArray = nil; g_tianDiPan_completion_handler = nil; g_tianDiPan_currentCoordinates = nil;
        return;
    }
    ...
    @try {
        ...
        if ([self respondsToSelector:action]) { SUPPRESS_LEAK_WARNING([self performSelector:action withObject:fakeGesture]); } 
        else { LogMessage(EchoLogError, @"[模拟器] 触发失败: Target 无法响应"); [self processTianDiPanQueue]; }
