////// Filename: Echo_AnalysisEngine_v13.23_Final_UI_Fix_TimeExtract_Integrated.xm
// 描述: Echo 六壬解析引擎 v13.23 (UI定稿修复版 v1.1)
//      - [FEATURE] 集成 v2.5 版本的精准时间提取逻辑，通过拦截弹窗获取时间，替换旧的直接提取方式。
...
// [MODIFIED] 完整替换 Tweak_presentViewController 函数
...
- (void)extractTimeInfoWithCompletion:(void (^)(void))completion {
    ...
    if ([self respondsToSelector:showTimePickerSelector]) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            for (int i = 0; i < 50; i++) { // Timeout after 5 seconds
