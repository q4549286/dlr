////////// Filename: Echo_AnalysisEngine_v12.4_UnifiedNotify.xm
// 描述: Echo 六壬解析引擎 v12.4 (全局通知统一版)。此版本将非阻塞式顶部通知应用到所有成功的提取任务，彻底替代UIAlertController，统一了用户体验。
//       - [MODIFIED] “课体”、“九宗门”、“课传流注”、“行年参数”等所有单项和批量任务在完成后，均调用顶部通知进行反馈。
//       - [MAINTAINED] v12.3的所有功能、UI设计和编译修复被完整保留。
...
static void LogMessage(NSString *format, ...) { if (!g_logTextView) return; va_list args; va_start(args, format); NSString *message = [[NSString alloc] initWithFormat:format arguments:args]; va_end(args); dispatch_async(dispatch_get_main_queue(), ^{ NSDateFormatter *formatter = [[NSDateFormatter alloc] init]; [formatter setDateFormat:@"HH:mm:ss"]; NSString *logPrefix = [NSString stringWithFormat:@"[%@] ", [formatter stringFromDate:[NSDate date]]]; g_logTextView.text = [NSString stringWithFormat:@"%@%@\n%@", logPrefix, message, g_logTextView.text]; NSLog(@"[Echo解析引擎] %@", message); }); }
...
static void Tweak_presentViewController(id self, SEL _cmd, UIViewController *vcToPresent, BOOL animated, void (^completion)(void)) {
    // [MODIFIED] 在S1提取中，也用新通知替换了旧的弹窗逻辑
    if (g_s1_isExtracting) { if ([NSStringFromClass([vcToPresent class]) containsString:@"課體概覽視圖"]) { vcToPresent.view.alpha = 0.0f; animated = NO; void (^extractionCompletion)(void) = ^{ if (completion) { completion(); } NSString *extractedText = extractDataFromSplitView_S1(vcToPresent.view, g_s1_shouldIncludeXiangJie); if ([g_s1_currentTaskType isEqualToString:@"KeTi"]) { [g_s1_keTi_resultsArray addObject:extractedText]; LogMessage(@"[解析] 成功处理“课体范式”第 %lu 项...", (unsigned long)g_s1_keTi_resultsArray.count); [vcToPresent dismissViewControllerAnimated:NO completion:^{ dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{ [self processKeTiWorkQueue_S1]; }); }]; } else if ([g_s1_currentTaskType isEqualToString:@"JiuZongMen"]) { LogMessage(@"[解析] 成功处理“九宗门结构”..."); [UIPasteboard generalPasteboard].string = [NSString stringWithFormat:@"// 九宗门结构\n\n%@", extractedText]; LogMessage(@"[完成] 内容已同步至剪贴板。"); 
        // 使用新通知
...
- (void)createOrShowMainControlPanel {
    ...
    titleLabel.text = @"Echo 六壬解析引擎 v12.4";
    ...
    for (int i=0; i<coreButtons.count; i++) { NSDictionary *config = coreButtons[i]; UIButton *btn = createButton(config[@"title"], [config[@"tag"] integerValue], [UIColor colorWithWhite:0.35 alpha:1.0]); btn.frame = CGRectMake(15 + (i % 2) * (btnWidth + 10), currentY + (i / 2) * 54, btnWidth, 44); [scrollView addSubview:btn]; }
    ...
    for (int i=0; i<auxButtons.count; i++) { NSDictionary *config = auxButtons[i]; UIButton *btn = createButton(config[@"title"], [config[@"tag"] integerValue], [UIColor colorWithWhite:0.5 alpha:1.0]); btn.frame = CGRectMake(15 + i * (btnWidth + 7.5), currentY, btnWidth, 44); [scrollView addSubview:btn]; }
    currentY += 54;
    ...
    g_logTextView.textColor = [UIColor colorWithRed:0.8 green:0.8 blue:0.8 alpha:1.0];
...
%new
- (void)copyLogAndClose { if (g_logTextView && g_logTextView.text.length > 0) { [UIPasteboard generalPasteboard].string = g_logTextView.text; LogMessage(@"日志内容已同步至剪贴板。"); } [self handleMasterButtonTap:nil]; }
%new
- (void)handleMasterButtonTap:(UIButton *)sender {
    ...
    if (g_s1_isExtracting || g_s2_isExtractingKeChuanDetail || g_isExtractingNianming || g_extractedData) { LogMessage(@"[错误] 当前有任务在后台运行，请等待完成后重试。"); return; }
    // [MODIFIED] 统一所有成功操作的反馈为新通知
    switch (sender.tag) {
...
- (void)extractSinglePopupInfoWithTaskName:(NSString*)taskName {
    LogMessage(@"[专项分析] 任务启动: %@", taskName); [self showProgressHUD:[NSString stringWithFormat:@"正在分析: %@", taskName]];
...
- (void)extractNianmingInfoWithCompletion:(void (^)(NSString *nianmingText))completion { g_isExtractingNianming = YES; g_capturedZhaiYaoArray = [NSMutableArray array]; g_capturedGeJuArray = [NSMutableArray array]; UICollectionView *targetCV = nil; Class unitClass = NSClassFromString(@"六壬大占.行年單元"); NSMutableArray *cvs = [NSMutableArray array]; FindSubviewsOfClassRecursive([UICollectionView class], self.view, cvs); for (UICollectionView *cv in cvs) { if ([cv.visibleCells.firstObject isKindOfClass:unitClass]) { targetCV = cv; break; } }
    if (!targetCV) { LogMessage(@"[行年] 未找到行年单元，跳过分析。"); g_isExtractingNianming = NO; g_extractedData = nil; if (completion) { completion(@""); } return; }
...
// 5. 构造函数
// =========================================================================
%ctor { @autoreleasepool { MSHookMessageEx(NSClassFromString(@"UIViewController"), @selector(presentViewController:animated:completion:), (IMP)&Tweak_presentViewController, (IMP *)&Original_presentViewController); NSLog(@"[Echo解析引擎] v12.4 (UnifiedNotify) 已加载。"); } }
